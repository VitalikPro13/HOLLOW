...
One property follows from the field sitting outside the profile signature and is stated here plainly: the signature on each entry prevents forgery, not deletion. A party able to rewrite an unencrypted profile announce in flight can strip the field, and the receiver reads that as a clear until the holder's next genuine announce. The planned remedy is a second signature by the master key over the field itself, required from an identity once a signed announce from it has been seen.
