...
- **Redeem (client, `api/shop.rs`):** the chain is verified (`verify_chain`) BEFORE the code is spent; after `finish_request` the assembled entry is verified again exactly as a viewer will; the credential is stored (`support_creds_own`) and republished BEFORE the pack is fetched, so a network failure after the burn costs a download (also behind the 30-day download link in the order mail), never the mark.
...
- **One chip per profile**, never one per credential: the compact card would run into the avatar's overhang. The icon alone plus a count past one, on every size (2026-09-26, Vitalik: the full profile's "Supported VitalikPro13" ran too long beside the Twitch chip); hover lists "artist: title" per piece.
...
- **The shop builds a missing pack at redeem** (`redeem.js` `ensurePack`): a listing approved before packs were built at approval (or whose pack left the disk) gets one from its own processed files through the encoder's passthrough, BEFORE anything is spent; failure is quiet (the credential is the deliverable, the pack the courtesy, and the buyer's Ko-fi download plus the order mail's link carry it too).
- **The shop's own `redeem_codes` table is the only authority on what a code is worth (since 2026-09-16), and `burnKey` is the ONE lock.** The rail's activation as a second lock is gone with the rail; the M4 order is unchanged: refused, burned, sign in memory, burn, count.
...
shop redeem code the shop validates a Twitch OAuth access token and signs what
...
## Codes since 2026-09-16: the Ko-fi model

The Creem rail (product, checkout, license keys, split, refund webhooks) is gone from `anonlisten-sites/shop`; plan and contract in `reports/shipped/shop/ARTIST_SHOP_DESIGN.md` section 10, memory `project_shop_kofi_code_model`.

- **Artist setup (`/artist/settings`, "Your Ko-fi"):** the artist pastes their Ko-fi verification token; the shop stores `HMAC(token)` (`artists.kofi_token_hmac`, host HMAC key) and mints a 32-byte webhook path stored as HMAC (`kofi_hook_hmac`, unique index = the lookup) plus a sealed copy (`kofi_hook_enc`, AES-GCM under the ENC key) so the card can show `<origin>/api/kofi/<hook>` again. Optional `kofi_forward_url` (https, no IP literal, no localhost, redirects never followed) for an artist whose one Ko-fi webhook slot is taken. `kofi_last_at`/`kofi_last_outcome` = the card's readout. Disconnect clears all six and UNPUBLISHES the artist's live pieces; saving a token again re-publishes the ones still carrying a link.
- **Webhook (`routes/api/kofi/[hook]`, `kofi.js` `ingestKofiEvent`):** 64 KB content-length cap before the read; form field `data` = JSON; artist by hook HMAC (404); token compared in constant time (401, "not set" is a refusal too); idempotent on `webhook_events` id `kofi:<message_id>` (200 duplicate); the stored event is a WHITELIST spine (message id, type, timestamp, transaction id, amount, currency, `shop_items[{direct_link_code, quantity}]`), never email, name, message, shipping or Discord fields; `type !== 'Shop Order'` = 200 and nothing minted; each line matched by `listingByKofiCode(artistId, code)` (this artist only, live or paused), quantity clamped 1..10, 20 lines max; `mintCodes` = one transaction with `sold += n`; codes `HOLLOW-XXXXX-XXXXX-XXXXX-XXXXX` (Crockford base32, 100 bits, 5 bits per char straight off `randomBytes`), stored as SHA-256 in `redeem_codes`; the buyer's email is used for ONE send (`orderMail`: codes as text and `hollow://redeem/` links, item page, a 30-day `/download/<token>` pack link from `pack_links`) and dropped; a failed send seals `{to, items}` into `order_mail_pending` (retried on admin loads after 5 min, every 20 min, deleted on success; the overview has "Send again"); Ko-fi still gets a 200 because a retry would mint twice. Forward copy after the answer, 10 s abort, never awaited.
- **Redeem (`redeem.js`):** `resolveListing` reads `db.listingIdForCode(sha)`; no network; `burnKey` is the one lock. Refunds: Ko-fi sends none; the keeper refuses a code by hash on `/admin/listing/<id>` (`refuseCode` -> `keys_refused`).
- **Env:** `SHOP_MODE=test|live` (live refuses `SMTP_DEV_SINK`); `ARTIST_EMAIL_HMAC_KEY` and `ARTIST_EMAIL_ENC_KEY` are ALWAYS required. Local drive: `scripts/kofi_send.mjs --hook <url> --token <t> --code <ko-fi item code> [--qty N] [--fresh]` (a flag given twice takes the FIRST value).
