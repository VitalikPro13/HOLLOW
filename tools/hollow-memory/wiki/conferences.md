...
- `startMeeting` → FFI start + `voiceChannelJoin('conf:x','main')`; `onAdmitted` does the same joiner-side. Both go through `_takeSeatOrGiveUp` (2026-09-26): host and joiner sit in `admitted` ("Joining the meeting", a delayed spinner and Leave) until the voice room SEATS them, and only then flip to `inCall`. When the seat never comes (20 s) or the join throws, the meeting is ended or left again with a `friendlyError` toast. It stays quiet when the voice side already said why (a 1:1 call). `startMeeting` and `requestJoin` check TURN first (`ensureTurnForCallFromRef`). A seat that lands after Leave is given back.
- Room list: `roomsLoaded` gates the empty state and `roomsError` holds a first-load failure (Try again = `loadRooms`). The waiting room reads `overallConnectionProvider`: offline drops the spinner and says the knock goes again on reconnect (Rust `reknock_if_pending`). `_clearConfChat(confId)` = clears the RAM chat key AND `voiceChannelProvider.clearServerParticipants(sid)` — called on start/requestJoin(fresh)/end/leave/onEnded/onKicked.
