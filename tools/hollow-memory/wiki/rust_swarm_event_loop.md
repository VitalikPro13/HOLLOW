...
- `DmSyncRequest` — (friend↔friend) queries DB for `is_mine=1` DM messages in the requester's conversation since a timestamp, responds with `DmSyncBatch` via Olm. Item packing factored into `build_dm_sync_items()` (reactions + file-meta batch joins), shared with the sibling responder below.
