...
- `CheckPendingJoinTimeout` -> `sync_handler::handle_check_pending_join_timeout()`: the 15s live window elapsed; PARKS the join (deposits into the `~join` ring, emits `ServerJoinParked`), never fails it. `ServerJoinFailed` has no emitter anywhere any more.
