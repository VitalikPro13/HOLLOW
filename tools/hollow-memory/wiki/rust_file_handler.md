...
## handle_request_file() and the pending ask table (node/file_asks.rs)

`file_handler.rs:handle_request_file()` -- Handles `NodeCommand::RequestFile` (the Download button, the DM chat-open sweep, the channel viewport sweep, a guest pull). Since 2026-09-05 ("honest file card states", tmp.txt item 1, memory `project_file_card_honest_states`) it is the front door of the asker-side PENDING ASK TABLE: whenever we hold a row for the file it upserts a `PendingFileAsk` (context read off OUR row: DM counterparty master, or server+channel; plus the sender master; ONE store open, raw-key SQLCipher so cheap) and dispatches to ONE holder. Without a row (a guest pull) it keeps the old single direct send.

**Multi-device (CRITICAL):** `peer_id_str` is the conversation MASTER, which no socket authenticates as. Candidates are DEVICE ids (`crypto_handler::online_devices_for`), ascending, and exactly ONE is asked at a time: every holder re-encrypts its stream with its OWN random AES key, the receiver keeps one FileHeader key, so two parallel streams can only fail AES-GCM decrypt and loop (the old "stuck at 16.2/16.2 KB" bug).

**Candidate order** (`file_asks::ask_candidates`): the explicit device from the command when live; DM = the counterparty's online devices, then our own online siblings (fanned the same bytes at send time); channel = the sender's devices, then `channel_holder_candidates()` (the renamed `channel_fallback_holder`: every online device of a member that passes `channel_readable_by`, the ONE home of that predicate). Caps: 256 asks, 4 holders per connection, 8 dispatches per sweep, `FILE_ASK_TIMEOUT_SECS` 15 (1 under cfg(test)).

**Every dispatch RE-STAMPS `requested_file_receipts` and clears `declined_file_ids`.** The receipt is cleared on `WsEvent::Disconnected` and CONSUMED at header time, so a retry that did not re-stamp would have its own answer refused by the auto-download gate, which is exactly the silent failure the table exists to end. The harness bounces the asker's socket between the command and the retry to make this load-bearing.

**The negative answer.** The FileRequest RESPONDER (swarm.rs) answers `HavenMessage::FileUnavailable { file_id, reason: gone | expired }` (`file_unavail`, plaintext like `file_req`) ONLY after the same entitlement gate the positive answer passes and only when a row exists; blocked, unknown id and non-entitled requesters keep getting silence (an answer to "unknown" would let a stranger tell "holds a row for X" from "does not": a membership leak). Not servable = `expired_at` set, `disk_path` None, or the read fails.

**Ingest (`file_asks::handle_file_unavailable`)**: HARD DROP unless the sender device is in the ask's `asked` set (the asset rail's asked-set rule). `gone` clears `in_flight` and rotates to the next candidate; nobody left = the card hears `gone` naming the sender's master (when one of its devices answered) else the last answerer. `expired` is verified LOCALLY first (channel row, our CRDT `retention_files` parses to days, our row's `created_at` past the window) before `mark_file_expired` on our own row; otherwise it is downgraded to `gone`. Wiki `security_write_gates` has the row.

**Retry points:** `PeerJoined` (a DM counterparty device or own sibling, or a member of the ask's server room), `RoomMembers` (every ask whose candidate list is now non-empty), the `asset_retry_timer` tick (a stale `in_flight` is a silent miss from an old client or a dropped frame: rotate, and announce `gone`/`waiting` when nobody is left; also retires asks whose row completed by another route), `Disconnected` (keep every entry, clear `asked`/`negatives`/`in_flight`). A settled ask keeps its entry so a newly joining device re-walks it; an explicit RequestFile on a settled ask resets the walk (the user asking again). The answering FileHeader (the explicit-pull path in `handle_incoming_request`) retires the ask.

**Cancel (`NodeCommand::CancelFileRequest` / FFI `cancel_file_request`, 2026-09-06):** the hover bar mirrors the card, so "Stop waiting for this file" calls `file_asks::cancel`, which drops the pending ask AND the file's `requested_file_receipts` entry. The receipt goes on purpose: an answer already on its way then arrives as an ordinary unsolicited push and the size cap plus the auto-download gate judge it as one. `declined_file_ids` is untouched (it records what the SETTING did to a push, not what the user did to a request), and nothing is emitted because Dart clears the card on the tap and an event would race it. Guard test `cancel_file_request_drops_the_queued_ask_and_its_receipt` (the holder returning proves the queue is gone; the refused answering header proves the receipt is).

**Events to Dart:** `NetworkEvent::FileAvailability { file_id, state, peer_id }`, state `requesting` | `waiting` | `gone` | `expired`, `peer_id` the MASTER the state is about. Dart's `lib/src/ui/chat/file_card_status.dart` is the single decision point for caption + control on every file surface.

**Retention unit bug fixed the same day:** `files.created_at` is MILLISECONDS (`order_us / 1000`), the sweep's cutoff is SECONDS, so `find_expirable_channel_files` never matched and channel-file retention had never fired. It now normalises units and also returns never-fetched rows (marked, not deleted); every `get_missing_*` query skips `expired_at IS NOT NULL`.

Guard tests: `dm_file_request_gets_honest_gone_answer_when_holder_lost_bytes`, `dm_file_request_waits_for_offline_holder_then_fetches_on_return`, `file_unavailable_from_unasked_device_changes_nothing`, `file_unavailable_never_answers_a_non_entitled_requester`, `channel_file_request_rotates_to_next_holder_after_gone`, `expired_answer_is_verified_locally_before_marking_our_row`, `retention_query_includes_never_fetched_rows`, `cancel_file_request_drops_the_queued_ask_and_its_receipt`, plus the older `channel_file_request_reroutes_to_online_holder_when_sender_offline`. Fleet: `scripts/fleet_file_card_states.ps1`.
