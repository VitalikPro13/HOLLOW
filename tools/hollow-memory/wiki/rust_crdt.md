...
**Reads are answered OUTSIDE the transaction.** The drain loop now collects the whole batch first, splits reads from writes, answers the reads, and only then opens the transaction — a batch that is all reads never opens one at all. `begin_transaction()` is `BEGIN IMMEDIATE`, i.e. a RESERVED write lock, and on iOS the DB lives in the App Group container where being suspended holding a lock is what RunningBoard kills the process for (`EXC_CRASH 0xdead10cc`).

**`CrdtStoreCmd::ChannelWatermarks { server_id, channel_ids, reply: oneshot }`** — the actor's ONE read command, exposed as `pub async fn channel_watermarks()`. Returns the newest stored message timestamp (ms) per channel; channels with no messages are simply absent. It exists because `sync_handler::catchup_watermark_age_secs` used to run a TRANSIENT `MessageStore::open()` **per channel, inline on the swarm event loop**, at every WS connect and channel open. A fresh connection has no schema cache, so its first PRAGMA re-reads and re-parses the entire schema (`sqlite3ReadSchema` → `sqlite3RunParser`) while holding a file lock — the TestFlight 0.9.5(50) iOS crash. The replacement, `sync_handler::catchup_watermark_ages()`, batches the whole server into one awaited round trip on the actor's warm connection and returns pairs in input order so the emitted `TopicCatchup` requests stay deterministic. Covered by `crdt_store::tests::channel_watermarks_answers_from_the_actor_connection` plus the existing `channel_relay_catchup_*` harness tests.
