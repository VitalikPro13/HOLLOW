...
- **Kinds** `AssetKind::{Emote, Banner, Sticker, Gif}` — differ ONLY in receipt cap (`recv_cap`: 256 KB / 1 MB / 512 KB / 2 MB) and per-request hash bound (`max_request_hashes`: 20 / 2 / 8 / 4). `emote_blobs.kind` TEXT column (additive migration, default 'emote') records what a blob was pulled AS.
