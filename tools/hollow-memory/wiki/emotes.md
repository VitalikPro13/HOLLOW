...
- **Kind never rides the wire.** Requesting side records hash→kind in swarm's `pending_asset_asks: HashMap<String, PendingAsk>` (the `kind` field; since 2026-09-05 the table outlives the socket and only each entry's `asked` set clears on `WsEvent::Disconnected`, see "Asset pull retry" below). `handle_emote_assets` accepts ONLY hashes present in that map and sizes the cap from the RECORDED kind — unsolicited bundles are dropped wholesale (cache-stuffing gate, new in this phase), and an invalid answer from the device we asked rotates the ask to the next holder (see "Asset pull retry").
...
- **Authoring**: the Banner row on server settings Overview (`ui/server_settings/pages/overview_page.dart`, desktop and phone; crop 3:1; GIF/animated-WebP picks BYPASS the crop — it flattens to a still — with a 2 MB pre-check). Stages optimistically + spinner + toasts.
...
**EVERY decode in the crate goes through `image_convert::load_bounded` (PROFILE-1, 2026-09-03).** `image::load_from_memory` appears nowhere any more — all 25 call sites were replaced. `load_bounded` builds an `ImageReader`, sets `Limits::max_alloc = MAX_DECODE_ALLOC_BYTES` (256 MiB) and decodes; `ImageReader::decode` runs `limits.reserve(decoder.total_bytes())` BEFORE the canvas buffer exists, so a header declaring a huge image is a refusal instead of an allocation. It is an ALLOCATION bound and deliberately not a dimension bound, because the same decoders serve local authoring where a 6000x4000 photo (91 MiB of RGBA) is ordinary. Note the image crate's own default was already 512 MiB, so this tightens an existing bound rather than creating the first one.

The DIMENSION ceiling `MAX_DECODE_DIM` (4096/side) is the separate, tighter REMOTE rule, applied by `image_convert::validate_remote_image_header` (header-only: format allowlist PNG/JPEG/GIF/WebP, `webp_header_dimensions` for WebP including the animated VP8X container that libwebp rather than the image crate decodes, else `ImageReader::into_dimensions` under the same limits). Its call sites are the places bytes arrive from somebody else: the profile still receipt (`social::gated_profile_image`), the pack importer (`hollowpack::blob_shape`, which also treats an unreadable header as a refusal because it decodes unconditionally two lines later), and the Klipy/Tenor CDN fetch (`api/gifs::media_fetch_and_store`, on `raw` before the transcoder). The asset rail (`emotes::handle_emote_assets`) applies the ceiling through `webp_header_dimensions` directly with `is_none_or`, NOT through the validator: its contract is container magic plus content hash with no decode, and `asset_cap_enforced_per_kind` pins that with a `RIFF`+`WEBP`+zeros fixture, so an unreadable header there must stay acceptable. Anything reached only from a local file picker keeps the allocation bound alone.
...
- **`GifThumbCache`** (`core/services/gif_thumb_cache.dart`): RAM ONLY since 2026-09-24 (200 entries AND 48 MB, LRU) + inflight dedup; the old 200 MB disk tier (`<app cache>/gif_thumbs`) held only stale previews, so `purgeLegacyDiskCache()` deletes it at every launch from `main()`; ONE shared keep-alive HttpClient + 4-slot FIFO download semaphore (unbounded parallel thumbs saturated the link AND Hostinger's PHP workers); client recreated on download exception (wedged keep-alive sockets). No Storage Manager segment any more: what a person keeps is an asset blob, counted under "Emotes & GIFs".
...
- **`ui/server_settings/pages/emotes_page.dart`** — server settings "Emotes and stickers" page (view for all, add/remove gated on `Permission.manageEmotes`, 50 cap, removal confirms inside the dialog); desktop and phone. Adds and removes show at once through `AssetWritesNotifier` (`serverEmoteWritesProvider` by name, `serverStickerWritesProvider` by hash, drawn over the stored list by `overAssetWrites`): the FFI only queues the op, so the page's own writes win until a refetch shows them, a failed write undoes itself, and one refetch `settleAfter` (2 s) decides a write the node refused later. See wiki `ui_server_settings`.
- Composer entry points (2026-09-24): the ONE expression button inside the composer opens `showExpressionPicker` (desktop popover) or, on the phone, swaps the keyboard for the public `ExpressionPanel` in a `MobileKeyboardPanelDock`; its Emoji tab is `EmojiPickerBody`. A pick goes to `_insertEmojiAtCursor` (desktop) / `_insertAtCursor(refocus: false)` (phone, so the keyboard does not return over the panel).
...
**Mobile server-emote management**: the same `EmotesPage`, pushed from the phone's server settings list at touch density.

Tests: `test/emote_composer_test.dart` (expansion, placeholder mapping, trigger rules, acceptance).

## Notification Previews (2026-07-11, plain-text path rebuilt 2026-09-14)

Every notification surface must run message text through a preview helper, never print raw text (tokens show as `[e:name:<64-hex>]` garbage):
- **In-app (renders images):** `emote_image.dart::emotePreviewSpans(text, style)` → `TextSpan` pieces + `WidgetSpan(EmoteImage)` sized to the line. Used by desktop `NotificationOverlay._MessageRow` + `MobileInChatBanner`, each wrapped in `EmoteScope(serverId: card.serverId, peerHint: card.peerId)`; the message is from a chat the user does NOT have open, so bytes are usually uncached, shows `:name:` and flips to the image when the pull lands.
- **Plain text (OS toasts / push):** `messagePreviewText(text)` (`lib/src/core/message_preview.dart`) → `:name:` for an emote token, plus every other wire token (`[file:...]`, `[a:g:...]`, `[a:s:...]`) resolved to a word. The old dedicated `emoteTokensToShortcodes(text)` is gone; `messagePreviewText` is the ONE helper now, and it forbids no other tokens leaking either. Choke points: `DesktopNotificationService.showDm/showChannel`; mobile `_showNotification`/`_showChannelNotification` in push_notification_service (covers lifecycle-background + FCM fetch paths). iOS killed-state NSE bodies still come from Rust, which has no Dart to call: `node/emotes.rs::emote_tokens_to_shortcodes` (manual scanner over `parse_emote_token`, unit-tested) applied in `push_enrich.rs` where the NSE JSON is built; that Rust path is unaffected by the Dart rebuild. Full rules (what each token becomes, `singleLine`, the `test/message_preview_guard_test.dart` CI guard): `lib/src/core/message_preview.dart`'s own doc comment.
...
## Asset pull retry (2026-09-05)

The requesting side no longer asks once per connection (that rule left a ring-replayed GIF, sticker or emote token a placeholder forever whenever its holder was away or the roster had not arrived yet). swarm.rs holds `pending_asset_asks: HashMap<hash, PendingAsk { kind, server_id, peer_hint, asked: HashSet<device>, first_asked_at, last_asked_at }>` (cap 512, oldest `first_asked_at` evicted). An entry leaves only when its blob is stored; `WsEvent::Disconnected` clears each entry's `asked` set and nothing else.

- **Candidates** (`emotes::ask_candidates`): the DM sender's live devices (`online_devices_for(peer_hint)` + `ws_room_for_peer`, the same shape `send_raw_to_identity` uses) and the members of `ws_room_peers[server_id]`, each sorted ascending for reproducibility, self and already-asked devices removed. ONE candidate per ask per dispatch; hashes batched per (target, room, kind) into one `EmoteRequest` (`dispatch_asks`); at most `MAX_ASK_CANDIDATES` = 4 devices per hash per connection; at most `MAX_ASKS_PER_SWEEP` = 8 hashes per sweep, because picking a holder walks the room tables on the event loop.
- **Triggers:** `WsEvent::PeerJoined` (`retry_asks_for_peer`: asks whose room is that room, or whose `peer_hint` resolves to the joiner's master), `WsEvent::RoomMembers` (`retry_asks_in_room`: the boot-ordering fix, the ring replays before the roster arrives), the 20 s `asset_retry_timer` (`retry_stale_asks`; `ASSET_RETRY_SECS` is 1 under `cfg(test)`; one store open per tick, skipped when the table is empty), and a `missing` reply.
- **Wire:** `HavenMessage::EmoteAssets` gained `#[serde(default)] missing: Vec<String>`. `handle_emote_request` now always answers an ask that carries at least one valid hash and lists what it lacks (the bundle may be empty; an old receiver decodes it as nothing). `handle_emote_assets` rotates a `missing` hash to the next candidate ONLY when the reply's sender is in that ask's `asked` set; anyone else's `missing` changes nothing. Old responders stay silent on a total miss and the timer covers them. Refused bytes (over the recorded kind's cap, not WebP, over the canvas ceiling) rotate the same way, gated on the same `asked.contains(sender)` rule, so a stranger's garbage can no longer delete a pending ask. A local re-request naming a DIFFERENT kind re-aims the surviving entry (new `kind`, `asked` cleared) and dispatches at once, which is what `asset_cap_enforced_per_kind` relies on (refused as emote, re-asked as gif). The receive gate is unchanged: unsolicited hashes drop, the cap comes from the recorded kind (`security_write_gates` row for `save_asset_blob`).
