...
**Multi-device device→master resolution — the iOS cold-tap fix (2026-07-02, awaiting device confirm).** A DM push `sender` can be the friend's DEVICE id, but every DM thread/provider keys on the MASTER. The 2026-06-20 attempt used the plain `identityFor` FFI — which reads ONLY the in-memory resolver, and swarm.rs warms that resolver *asynchronously* inside the spawned event-loop task. A buffered cold-start tap fires the instant MobileShell mounts (before the warm), so the device id resolved to ITSELF with **no error** — the `deviceLinkProvider` fallback never triggered → device-keyed empty thread. (The NSE always filed the same DM correctly because it warms from the `device_links` table directly — proof the link IS on disk at tap time.) Fix: `_openChatFromPush` now calls **`identityForPersisted`** (`identity_for_persisted` in api/network.rs, next to `identity_for`): in-memory resolve first; if it returns the input unchanged, open SQLCipher (same identity/passphrase pattern as `get_push_profile`), `warm_from_links`, resolve again; any failure degrades to passthrough. Logs `[HOLLOW-PUSH] identity_for_persisted: <raw> -> <resolved> (memory|db)` to hollow_debug.log — if raw==resolved on a `(db)` line, the link genuinely isn't in `device_links` on that phone (then the fallback plan is carrying the master in the push payload / NSE userInfo). Android was always fine (it taps the Dart-posted local banner whose payload is already the resolved master `personKey`); channels are identity-independent (`server:channel`).
...
## Android nudge-the-live-node path (2026-07-03)

Android keeps the FULL node registered while backgrounded, so `start_fetch_node` refused ("Full node is running") and every backgrounded push degraded to the placeholder. The handler now tries a nudge first, for BOTH wake types:

- DM: `nudge_live_dm_fetch(sender)` (api/network.rs) — resolves masters, computes `dm_room_code`, sends `NodeCommand::JoinRoom` to the LIVE node. Channel wake: `nudge_live_room_join(server)` joins the server room. Ok(false) = no node running → normal fetch-node path.
- The relay replays buffered ciphertext on room join; a doze-killed zombie WS reconnects via the queued command. The live node decrypts + persists + emits; the MAIN isolate posts the real notification through normal routing (mute/prefs/mentions + message-id dedup respected).
- The handler only CONFIRMS delivery by polling SQLCipher (`_waitForLiveDmArrival` / `_waitForLiveChannelArrival` in push_notification_service.dart: baseline max rowid, 500ms poll up to 10s, 60s already-arrived recency check) and stays silent on success. Placeholder = timeout fallback only.
