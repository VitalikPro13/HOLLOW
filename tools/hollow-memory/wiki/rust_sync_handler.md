...
**2026-09-05 (rung 2 + the short window):** `handle_join_server` mints the join's MLS KeyPackage ONCE per row through `crypto_handler::mint_key_package` (generate + `persist_mls_state`, the only mint site in `node/`) and stores it on `PendingJoin.key_package` / the `pending_server_joins.key_package` column; `deposit_parked_join` puts it on the PARKED ring copy only (a live request carries `None`). Two timers are armed per request: `JOIN_EMPTY_ROOM_WINDOW` (3 s, `CheckPendingJoinTimeout { only_if_empty: true }`) parks only if `ws_room_peers[server_id]` exists and holds nothing but our own ids; `JOIN_LIVE_WINDOW` (15 s) is the authority otherwise. `discard_join_key_package` (called LAST in `handle_join_refused` and `handle_discard_pending_join`) deletes the unused bundle via `MlsManager::discard_key_package` and persists. `request_channel_catchups` is the one definition of the connect-time channel catch-up, re-issued after the Welcome that ends a parked join. The `ChannelSyncRequest` / `ChannelSyncProbe` MLS/Olm twins (`handle_envelope_channel_sync_req` / `handle_envelope_channel_probe`) gate on `crypto_handler::channel_readable_by` (membership-or-public AND `can_see_channel`) before serving anything.
