...
- **Diagnostics (iron rule: frames>0 or the field test didn't happen):** `Helper.getNoiseSuppressAiActive()` → `available/enabled/ready/bailed/formatOk/active` + `frames/emaMs/engine/vad` + raw shape `rate/channels/bands/bufferSize`; services log the map at every capture and reconcile into hollow_debug.log. `vad: -1` = the chain gate is running on fallback gating.
