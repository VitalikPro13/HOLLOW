...
- **Mobile is a separate mechanism (2026-08-15).** The three Settings pickers are DESKTOP-only; a phone picks a ROUTE, not a device. `audioRouteProvider` + `core/services/audio_route.dart` own it, and both notifiers gained `selectAudioRoute(AudioRoute)` (sets `isSpeakerOn = kind != earpiece`, then delegates) alongside a rewritten `_setSpeakerRoute` that routes through `AudioRoutes.preferLoudRoute()` instead of `Helper.setSpeakerphoneOn` — **a connected headset always beats the loudspeaker**. Both call `audioRouteProvider.notifier.refresh()` after applying and `.reset()` in `_cleanup()`/`onLocalLeft()`. UI + the full rule: ui_mobile.md §Audio routing + device picker.
...
- **Settings:** `noiseSuppressAiProvider` (key `noise_suppress_ai`, DEFAULT false) + `noiseSuppressEngineProvider` (key `noise_suppress_engine`, 'rnnoise' default | 'dfn3'; `noiseSuppressEngineToNative` → 0/1). Toggles: desktop `audio_section._buildNoiseSuppressAiToggle` + a pair of `HollowChip`s with a `hint` (shown while ON); mobile toggle only (RNNoise). Engine switch mid-call = LIVE native handle swap (old handle deliberately leaks, latches reset) — no re-capture, no reneg; `updateNoiseSuppressEngine` on both services.
