...
## AI Noise Suppression (DFN3 scaffold, 2026-07-17/18) — NOT LIVE YET

Optional DeepFilterNet3 denoise at the HEAD of the native capture post-processor (post-AEC, the Krisp slot), engine-agnostic scaffold. **As of 2026-07-18 no live frame has ever been denoised**: Windows' capture hook runs at 16 kHz mono, Android's at 48 kHz split-band (3×160) — both rejected by the 48k-fullband gate, and the WebRTC-NS fallback re-arms automatically. Going live needs the adapter layer (ABI v2 `process_ex`: 16k resample + ThreeBandFilterBank merge); engine plan is dual (DFN3 desktop / RNNoise-`nnnoiseless` mobile). Full forensics: memory `project_dfn3_noise_suppression`.

- **Setting:** `noiseSuppressAiProvider` (settings_provider.dart, key `noise_suppress_ai`, DEFAULT false). Toggles: desktop `audio_section._buildNoiseSuppressAiToggle`, mobile `_VoiceEnhanceToggle` bottom row.
- **Services** (`voice_service` + `voice_channel_service`, kept behavior-identical): `noiseSuppressAi` pref + `_dfnFallbackNsOn` + `_wantWebrtcNs` drive the `noiseSuppression`/`googNoiseSuppression` getUserMedia constraints (legacy NS off while DFN owns suppression — double suppression = artifacts). `updateNoiseSuppressAi` = native toggle + SAME-DEVICE mic re-capture via the shared `_recaptureMic` (extracted from setAudioInputDevice; DM returns bool → caller renegs, VC renegs internally). `reconcileNoiseSuppressAi` (scheduled +4 s after toggle-on AND call/session start) re-arms WebRTC NS + re-captures when the engine can't run — a call must never sit with NO suppression.
- **Providers:** listeners guarded prev==next; DM `_applyNoiseSuppressAiChange` in call_provider (+ call-start check in `onConnected`); VC listener INSIDE the `_deviceListenersWired` once-guard (recapture is heavy like a device switch). Pre-warm: `hollow_shell._bootstrap` kicks `Helper.setNoiseSuppressAi(true)` when the pref is on — model load is 455 ms desktop but **15 s on a Pixel**.
- **Native scaffold:** `rust/hollow_dfn` engine crate (deep_filter GIT tag v0.5.6) + `hollow_core/src/dfn_ffi.rs` C ABI (outside `api/`, push_enrich pattern) + runtime binding per port — Windows `GetModuleHandleW("hollow_core.dll")`, Linux own `dlopen` (Dart's is RTLD_LOCAL), darwin `dlsym(RTLD_DEFAULT)`, Android `DfnBridge.java` `System.loadLibrary` (FRB's dlopen doesn't register with ART; JNI export names derive from that class's FQN). Hook at head of all 3 ports' process; realtime watchdog (EMA >6 ms → latched session bypass); handle created once on a background thread, immortal.
- **Diagnostics (iron rule: frames>0 or the field test didn't happen):** `Helper.getNoiseSuppressAiActive()` → `available/enabled/ready/bailed/formatOk/active` + `frames/emaMs` + raw shape `rate/channels/bands/bufferSize`; services log the map at every capture and reconcile into hollow_debug.log.
