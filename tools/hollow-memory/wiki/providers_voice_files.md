...
5. Loads mic gain from `micGainProvider` (default 2.0 = `kMicGainDefault`, falls back to `kMicGainDefault` not 1.0). Adds `ref.listen(micGainProvider)` for mid-session gain changes via `_service.updateMicGain()`.
