...
7. `_DockDivider`, then the tools: **Downloads** (`DownloadIconButton`, a `HollowIconButton` with a `HollowCountBadge` of transfers in flight), **Help** (`_HelpButton`, toggles `helpPanelOpenProvider`, `selected` while open), **Settings** (`toggleSettings(ref.read)`: Settings is a place, the gear carries the `NavSelectionMark` while it is open and it never folds into the Places menu).
