...
**The STUN-only guarantee is enforced by a dedicated transport (fixed 2026-08-03).** Share negotiates its OWN peer connection per peer (`ensureShareConnection` -> `connectShareToPeer`, `_Lane.share` in `webrtc_service.dart`) over the `rtc_share_*` envelopes, and never reuses the general `hollow-data` connection — which is TURN-capable and, with "Always relay calls" on, TURN-only. Remote `typ relay` candidates are refused locally too, so the guarantee does not depend on the peer's config. Previously the Share short-circuited onto the general connection whenever one existed (i.e. for anyone you chat with) and inherited its TURN candidates: memory `project_share_data_channel_reuse_turn`.
