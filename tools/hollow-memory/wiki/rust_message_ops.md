...
6. **Multi-device fan-out (Phase 6, Step 3)** — `fan_out_dm_envelope(...)` instead of a single send:
   ...
   - Per device, `send_dm_to_device()` runs the 3-way branch keyed by the DEVICE id: **EXACT-device** online (`ws_room_for_peer(...).is_some()`, NOT identity-wide `peer_is_reachable` — else an offline sibling whose other device is online takes the online path and gets dropped) → `send_encrypted_message()`; offline-with-session → encrypt to `dm_room_code` (push trigger) + queue in `pending_messages[device]`; no session → queue + KeyRequest. `pending_messages` is keyed PER DEVICE; the per-device drain on PeerJoined/RoomMembers/KeyBundle delivers it.
