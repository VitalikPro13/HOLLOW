...
6. **Multi-device fan-out (Phase 6, Step 3)** — `fan_out_dm_envelope(...)` instead of a single send:
   ...
   - **Self fan-out** to our own OTHER devices (siblings), excluding `device_peer_id`. Siblings get the `convo`-tagged variant so they file the echo under the right conversation, not under ourselves. Since #90 (2026-09-23) the sibling set comes from `collect_target_devices(Some(olm), ...)` like the recipient set, so OFFLINE siblings with a session are included.
   - Per device, `send_dm_to_device(..., is_sibling)` runs the branch keyed by the DEVICE id: **EXACT-device** online (`ws_room_for_peer(...).is_some()`, NOT identity-wide `peer_is_reachable` — else an offline sibling whose other device is online takes the online path and gets dropped). **Online RECIPIENT (`!is_sibling`): encrypt + send to the DETERMINISTIC `dm_room_code`** (NOT `send_encrypted_message`, whose `ws_room_for_peer` first-match over a HashMap can pick a room the recipient LEFT when co-present in >1 room → relay buffers against a dead room → SILENT one-way DM loss; fixed 2026-07-09). **Online SIBLING: `send_encrypted_message()` (flexible lookup)** — siblings meet in `inbox:{our_master}`, NOT `dm_room_code(M,M)`, so dm_room is WRONG for them (routing a sibling echo to dm_room regressed self-DM/saved-messages). Both online branches ALSO queue in `pending_messages[device]` (cap 20). Offline-with-session → encrypt to `dm_room_code` (relay buffers + push trigger) + queue, for a recipient AND a sibling (#90); no session → queue + KeyRequest. `pending_messages` keyed PER DEVICE; drained on PeerJoined/RoomMembers/KeyBundle/re-key. DM typing (social.rs) uses the same fix via `send_message_to_peer_in_room(dm_room, ...)`. See `feedback_dm_friend_establishment_bugs_2026_07.md`.
     - **Offline siblings get a relay-buffered copy (#90, 2026-09-23), without a push.** Until then an offline sibling only queued in RAM (Step 5 fix against our own phone buzzing), so it depended on backfill with a device or the friend online at the same time. Now the sibling copy takes the same room-send as a recipient; the phone suppresses the wake-up at the RELAY by listing its own sibling device ids in the `~dm` no-push entry (`notification_provider._ownSiblingDeviceIds`, see push_notifications). `is_sibling` now only picks the ONLINE route (inbox room via `send_encrypted_message`). A push-woken fetch isolate skips own-sibling envelopes, so a drain there can consume the copy; the gap digest (rust_storage_db) heals it. Harness: `offline_sibling_gets_a_buffered_copy_of_an_own_send`.
