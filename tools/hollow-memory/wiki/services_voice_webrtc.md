...
## Linux: the audio device module's lifetime, and why the plugin holds an anchor PeerConnection (2026-09-09)

libwebrtc's connection context counts live PeerConnections; when the count drops to zero it
calls `WebRtcVoiceEngine::Terminate()` (StopPlayout, StopRecording, `adm->Terminate()`) and the
next PeerConnection calls `Init()` again. The PulseAudio module does not survive that cycle: after
the re-init `StartRecording()` times out after 10 s with `failed to activate recording`, and
playout the same, which is the 10 to 20 s freeze on the signalling thread and a call that connects
with no audio either way. Any closed connection before a call triggers it (the polite side of the
data-channel glare, a previous call, a share), and the glare tiebreaker is deterministic per pair,
which made it look like one machine was always broken. Windows and macOS modules re-init fine.

`FlutterWebRTCBase` (`packages/flutter_webrtc/common/cpp/src/flutter_webrtc_base.cc`) therefore
...
and the plugin's `initialize` method no longer replaces it when Dart passes the listener's default
severity "none". `WebRtcNativeLog.start()` (`lib/src/core/services/webrtc_native_log.dart`, called
from `main()` on desktop) re-routes the sink into hollow_debug.log as `[WEBRTC-NATIVE]` lines.

Unrelated to calls but found the same day: `registerVideoBackend()`
(`lib/src/core/services/video_backend.dart`) gives fvp software decoders on Linux because mdk's
hardware decode path (CUDA without libnvcuvid) crashed the process on the first Play tap; `main()`
and the UI probe both call it. Memory: `project_linux_call_audio_adm_init`,
`feedback_linux_fvp_hw_decoder_segfault`, `feedback_linux_stderr_epipe_panic`.
