...
Per-process window audio (Windows): `DesktopCapturerSource.pid` extracted from HWND via `GetWindowThreadProcessId` in the C++ source enumeration. PID flows through `ScreenShareSelection` → provider → exe `--pid` arg. Uses AUDIOCLIENT_ACTIVATION_TYPE_PROCESS_LOOPBACK INCLUDE mode (Windows 10 2004+). **Entire-screen anti-echo (LIVE 2026-06-30):** when no `--pid` (whole-screen), the exe is passed `--exclude-pid <hollow PID>` and uses `PROCESS_LOOPBACK_MODE_EXCLUDE_TARGET_PROCESS_TREE` to capture system audio MINUS Hollow's own (the Windows mirror of macOS `excludesCurrentProcessAudio`), so the remote no longer hears itself; graceful fallback to plain system loopback on Windows <2004. **KNOWN GAPS (NOT yet fixed — see tmpWindowsNext.txt):** (1) per-WINDOW capture currently sends full system audio because `selection.pid` arrives as 0 for window shares (and even a correct HWND PID is the wrong one for browsers/Electron, which render audio in a separate utility process); (2) the blunt exclude-self also drops Hollow's OWN legitimately-played media (in-app meme video). Both are fixed by the Discord-style per-app INCLUDE-set architecture: enumerate audio sessions (`IAudioSessionEnumerator`→`IAudioSessionControl2::GetProcessId`), resolve the window to its real audio PID(s) via process-tree ancestry, INCLUDE-capture a PID set, and mix. A working INCLUDE on a real PID yields only that tree's audio OR silence (never system-wide) — so "captures everything" proves the per-app path isn't engaging.
