...
`startScreenAudioCapture(streamId, {pid, windowHwnd, onPacket})`:
1. Creates `ScreenAudioCapturer` (`lib/src/core/services/screen_audio_capturer.dart`)
2. Spawns `screen_audio_capturer.exe --mode pipe [--window-hwnd <HWND> | --window-pid <PID> | --exclude-pid <hollowPID>] --duration 0`
3. Exe captures audio per the arg (per-app INCLUDE+mix for a window, system-minus-Hollow for entire-screen), Opus encodes, writes framed binary to stdout
...
**Entire-screen anti-echo (Windows, LIVE 2026-06-30):** for a whole-screen share (no window hwnd/pid) the exe gets `--exclude-pid <hollow PID>` and uses `PROCESS_LOOPBACK_MODE_EXCLUDE_TARGET_PROCESS_TREE` to capture system audio MINUS Hollow's own (the Windows mirror of macOS `excludesCurrentProcessAudio`), so the remote no longer hears itself; graceful fallback to plain system loopback on Windows <2004. **REMAINING GAP (Bug B, deferred — see tmpWindowsNext.txt):** this exclude-self also drops Hollow's OWN legitimately-played media (an in-app meme/video) — viewers hear every app except Hollow. Windows has no per-audio-session filter and Hollow's call-voice + media render from the same process, so the fix needs the call-voice playback in a SEPARATE process (then entire-screen = include-set of all render pids minus the call-voice child). Not yet built.
