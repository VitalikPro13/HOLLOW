...
| `user_profiles.support_creds` — the FIELD SIGNATURE and its per-master pin (2026-09-03) | `social::gated_support_creds`, called from `save_incoming_profile` ahead of the entry validator above; signed at every announce site by `crypto_handler::sign_support_creds` (`handle_update_profile`'s two frames, `send_own_profile_inner`, the swarm backfill announce) | The field sits OUTSIDE `profile_signing_payload` because each entry binds the identity by itself — which covers forgery and misses DENIAL: on the plaintext `HavenMessage::ProfileUpdate` fallback a relay rewrites the field to `""` in flight, the profile signature still verifies, and `Some("")` is the holder's explicit clear on every receiver. So the field carries its own master Ed25519 signature over `"hollow-support-creds-sig/v1" || len(peer):u16 BE || master || updated_at:i64 BE || field` (`support_creds::support_creds_sig_message`), verified with the profile's own `profile_pk` (re-derived to the master, so a real signature by somebody else is not one by this identity). THREE outcomes and no fourth: a VALID signature applies the field and sets the `user_profiles.support_creds_signed` pin; no valid signature from a PINNED master is a REJECT of the field (treated as ABSENT, preserve what we stored, logged once per master, never a clear — `if sig.is_some()` is not the bypass); no valid signature from a master that has never signed takes the legacy path (the entry validator alone), which is what keeps every shipped client working. Additionally the field is ignored whenever the announce's `updated_at` is STRICTLY OLDER than the stored row's, signature or not: `save_profile`'s own freshness guard tolerates 24 hours of backdating, so replaying a genuine older announce from before a purchase would otherwise clear the marks. The pin is set only AFTER a successful save and only ever forwards. Harness: `support_creds_sig_pins_and_a_stripped_field_is_preserved`. |
