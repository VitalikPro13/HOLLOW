...
- **`HollowTabBar<T>`** (`hollow_tab_bar.dart`, 2026-09-26): the ONE exception to chips, for a row that IS a surface's header and splits the whole surface into sections (expression picker, Friends manager): the same `HollowChipTab`s as equal tabs, a 2 px accent bar inset `lg` under the open one, 40 tall (48 `touch`, or `height:`), arrows/Home/End.
- **`HollowChipTabs<T>`** (`hollow_chip_tabs.dart`, `HollowChipTab(value, label, hint, count, icon)`): every OTHER tab row (dialog body, page, place header) is a `HollowChip` row, 8 apart, one selected; `hint` a quiet total, `count` a `HollowCountBadge` for something waiting on the person (`HollowChip` gained `count:` and `focusNode:`); Left/Right/Home/End move and select; `expand: true` equal widths on a phone, else it wraps. No other underline tab, no local `_Tab`. Users: screen share, game card tiers, Share, Archive, `place_header.dart`.
...
**The tab bar IS the header** (`HollowTabBar`, 2026-09-26; chips "felt out of place" there): "Friends"
(count as `hint`), "Requests" (`count:` badge while requests wait), "Add friend", flush to the
dialog's clipped top and left edges so a hover fills the rounded corner, the open tab's accent bar
on the hairline under it. The close X follows with even margins: the bar is exactly the X's
size + 8 above and below, then 8 after it (measured 17/18/17 px). No "Friends" title (it said
the word twice); the route still announces "Friends" via `Semantics`.
