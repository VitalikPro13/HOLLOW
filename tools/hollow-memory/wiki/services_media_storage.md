...
Used by: `TypingDots`, `AmbientBackground` and the mobile Chats header glow (selected rows are a flat fill; there is no shimmer). Section headers and dividers are static. `pulse` is GONE — `StatusDot` is always static.
...
**One `Timer.periodic(33ms)`** (`_ticker`, `_fastIntervalMs = 33`) drives both notifiers. `_onTick()` reads a `Stopwatch` and computes each value from `elapsedMicroseconds` modulo its cycle duration.

- **TypingDots:** `(us % 1200000) / 1200000` -- straight linear ramp.
- **Ambient:** `(us % 45000000) / 45000000` -- straight linear ramp.

`_syncFast()` starts the timer only while `typingDots` or `ambient` has a listener, and cancels it when the last one goes away. Writing a value nobody listens to is free, so both are computed unconditionally.
