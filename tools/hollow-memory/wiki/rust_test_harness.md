...
**2026-09-05 additions.** Helpers: `restart_node(relay, node, master_tag, device_tag)` (takes the node off the relay, aborts its loop via `TestNode::into_storage`, respawns on the SAME DB through `spawn_node_on_db`, which now LOADS the existing Olm account + sessions like production; MLS state is restored by `run_event_loop`), `expect_quiet_group` / `agreed_epoch` (an epoch unchanged across two reads 2500 ms apart), `MockRelay::set_recording` / `recorded_frames` (count what a device puts on the wire), the pre-existing `RawSocket` used as a leaf-less or non-member presence. Tests: `three_member_live_join_lands_at_minimal_epoch` (epoch 2, one KeyPackage per joiner, a leaf repair costs exactly two), `parked_join_key_package_survives_a_restart_before_the_welcome`, `parked_join_completes_with_zero_overlap` (extended: buffered Welcome asserted, ready with nobody online, a pre-return message decrypted by the joiner alone), `empty_server_join_parks_within_the_short_window`, `join_with_a_silent_member_present_keeps_the_long_window`, the four fan-out sweep tests (`member_added_and_pledge_ops_reach_a_deaf_member`, `vc_state_signal_reaches_a_deaf_member`, `channel_typing_and_profile_update_reach_a_member_without_a_leaf`, `channel_file_header_reaches_a_member_without_a_leaf`) and `restricted_channel_history_and_files_never_reach_a_non_qualifier`. Lesson: a deaf node still forms a leaf through the targeted Welcome and syncs state on co-presence, so the leaf-less member that discriminates is a raw socket admitted from the ring by a legacy request; assert on delivered frames. Known intermittent: `call_signal_routes_to_friend_device_and_drops_unknown` (a pre-existing two-round Olm glare, about 1 in 10).
