...
- **Ingest (`crypto_handler::ingest_device_list`):** verifies signature, rejects our OWN master (we're the authority for our own list), enforces strictly-increasing `version` vs `device_list_version()`, persists, `resolver::update_many`, emits `DeviceListUpdated`. Called from both `ProfileUpdate` receive handlers (envelope + bare `HavenMessage`).
...
Friend's UI still shows your devices as separate dots → Step 2 (cosmetic collapse via `device_link_provider`, deferred by user). Your own two devices do NOT share DM history + sending a DM to a multi-device friend doesn't fan out → Step 3 (Olm sender-side fan-out) + Step 5 (backfill). Freshly-linked device has no friends/servers → Step 4 (QR snapshot) + Step 6 (MLS per-device leaves). A fresh device CAN already receive DMs from single-device friends and friend-request normally.
