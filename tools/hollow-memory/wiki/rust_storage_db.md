...
Sources: `api/storage.rs` (`export_backup_bytes`/`import_backup_bytes`/`build_snapshot_bytes`/`stash_pending_link`/`has_pending_link`/`import_pending_link`), `node/link_handler.rs` (orchestration), `node/ws_stream_transfer.rs` (`StreamKind::LinkSnapshot`), `node/file_handler.rs` (completion→stash), `relay-uws/src/ws_handler.cpp` (`link_code` verbs), `lib/src/core/providers/device_link_sync_provider.dart`, `lib/src/ui/dialogs/device_link_dialog.dart`.
...
- **Import (the fix that made it work):** the receiver's completion arm calls `storage::stash_pending_link(blob, code)` → writes `pending_link.hollow` + `pending_link.code` to the data dir → emits `LinkComplete` → the UI auto-restarts. On NEXT launch `_bootstrap` (FIRST, before `hasIdentity()`) calls `has_pending_link()` → `import_pending_link()`, which deletes the throwaway identity files then `import_backup_bytes(blob, code)` — the IDENTICAL path as a manual "Restore from backup". Then bootstrap proceeds as a normal restored launch.
...
- **OPEN follow-up (pre-Step-5):** the FIRST DM a freshly-linked device sends to a friend doesn't mirror to its sibling (sibling self-echo fan-out targets a stale ghost device id because the live sibling isn't yet in the DM-with-friend room — only `inbox:{master}`). Every message after it works. See the tracker checkbox before Step 5.
