...
**Mic Gain** — Slider (83 divisions) indented below the Microphone row, with a caption line ("Boosts your outgoing voice… limiter at -3 dB prevents clipping."). Reads/writes `micGainProvider`, which is the **actual linear makeup multiplier** clamped to `kMicGainMin`=1.0 / `kMicGainMax`=3.0 in `settings_provider.dart`. The **default is 2.0** (`kMicGainDefault`) — a real 2× boost over WebRTC's quiet ~−18 dBFS AGC output, which is what makes calls audible with no setup. The slider RESCALES its display: `(gain / kMicGainDisplayUnit * 100)%` with `kMicGainDisplayUnit`=2.0, so 1.0x→"50%", **2.0x→"100%" (the normalized loud default)**, 3.0x→"150%". Drives the native **post-APM flat makeup gain + soft limiter** (`SoftLimit(sample*gain)`, the clean committed `FlutterCaptureGainProcessor`) via `Helper.setCaptureGain()` (NOT `setVolume`, which only scales remote tracks). The -3 dBFS soft limiter caps the 2× boost so it can't clip. Applies live mid-call. (A multi-stage adaptive DSP chain was tried 2026-06-28 and reverted — it crackled; the flat-gain processor + louder default is what shipped.)
