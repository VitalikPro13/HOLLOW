...
**Channel-open subscribe:** `initState` (channel branch) calls `network_api.subscribeChannels(serverId, [channelId])` — route-level so EVERY entry (Chats tab, banner, push tap) gets live topic broadcasts; the Chats-tab path previously never subscribed.
