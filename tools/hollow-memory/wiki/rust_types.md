...
- **`ServerJoinParked { server_id }`** (pending joins rung 1): the live window elapsed (3 s when the server room is empty, 15 s otherwise, 2026-09-05) with nobody there to answer. NOT a failure: the request is persisted locally and deposited into the server room's `~join` ring; the UI shows a pending tile.
...
- **`CheckPendingJoinTimeout { server_id, only_if_empty }`** — internal: check if a pending server join timed out (timer-driven). `only_if_empty` = the 3 s short window that parks only when the server room holds no other device (2026-09-05). Handler: swarm.rs.
