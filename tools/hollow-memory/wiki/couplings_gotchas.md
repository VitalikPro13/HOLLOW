...
**Where:** `lib/src/ui/server_settings/pages/channels_page.dart` -- `_ChannelPanel` (Who can see it / Who can post via `ChannelAccessPicker` from `channel_access_pickers.dart`, and the Public switch); desktop and phone share the page since 2026-09-24. Also `event_provider.dart:_refreshServerState()`.
...
**Where:** `rust/hollow_core/src/crdt/mod.rs` (resolver hook), `crdt/server_state.rs` (`is_member`/`canonicalize_members`/resolver-aware accessors), `node/swarm.rs` (PeerJoined gate, post-join bootstrap, MLS identity regen, member-key canonicalization), `node/crypto_handler.rs` (`send_raw_to_identity`, election collapse), `node/sync_handler.rs` + `message_ops.rs` + `vault_ops.rs` + `voice_handler.rs` + `file_handler.rs` (per-device send fan-out), `crypto/mls_manager.rs` (`credential_identity`, `remove_identity_leaves`), `api/storage.rs` (`import_pending_link` MLS clear), `lib/src/ui/shell/{hollow_shell,member_panel,user_bar}.dart`, `lib/src/ui/mobile/{mobile_shell,mobile_member_panel}.dart`, `lib/src/ui/server_settings/pages/members_page.dart`, `lib/src/ui/chat/channel_chat_pane.dart` (sender-grouping collapse). See memory `project_multidevice_step6_mls_leaves`.
