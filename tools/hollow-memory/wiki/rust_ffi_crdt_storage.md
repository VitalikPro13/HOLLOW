...
**`MessageStore::open()` is the universal open path (~139 call sites) and is cheap + concurrency-safe.** It sets the SQLCipher key, the journal mode (WAL everywhere except iOS=TRUNCATE — load-bearing for the App Group NSE, `0xdead10cc`), perf PRAGMAs, and creates tables. It does NOT run `VACUUM`/`auto_vacuum` migration — that one-time legacy conversion (`auto_vacuum=0` → INCREMENTAL, a full exclusive-lock file rewrite) lives in `MessageStore::migrate_auto_vacuum_once`, called ONCE in `api::network::start_node`'s single-connection window (before the first open + the long-lived `CrdtStore`/`CryptoStore` actor connections). Running the VACUUM/page-1 probe inside `open()` raced the SQLCipher codec across the concurrent connections and logged harmless-but-noisy `hmac check failed for pgno=1`. The harness mirrors the migration call in `spawn_node_with_friends` for parity. Likewise the swarm startup shard-integrity check (a separate `ContentStore` connection) is gated on `!server_states.is_empty()` — a serverless node has no shards to verify, so it skips that extra concurrent open. (A small residual hmac log remains from a concurrent-write interaction on the actor connections; harmless, all data correct.)
