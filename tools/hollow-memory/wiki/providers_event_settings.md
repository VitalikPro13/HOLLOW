...
- **Desktop:** the surface is chosen by `_useNativeToast()`. Native OS toast when the window is **hidden (tray) OR not provably focused**; the in-app overlay card ONLY when visible AND focused (focused-on-another-chat). When focused-on-this-exact-chat, `event_provider` never calls this (gated by `isViewingDm`).
- Avatar bytes fetched + cached via `getPushProfile` (`_avatarFor`).

**Focus must be PROVEN, not assumed (`_isWindowFocused`).** Two independent sources exist and are read in different places: `windowFocusedProvider` (event-driven, window_manager `onWindowFocus`/`onWindowBlur` from `WM_NCACTIVATE`) drives the gate in `event_provider`, while `windowManager.isFocused()` (live `GetForegroundWindow` compare) is a direct query. `_isWindowFocused()` returns `nativeFocused && eventFocused`, and a query that THROWS counts as not focused. The branches are not symmetric -- a toast is always visible, an in-app card is invisible the moment another window is on top -- so ties go to the toast. On disagreement it logs and writes a native `false` back into `windowFocusedProvider` (one-way ONLY; a wrong native `true` is the failure being defended against and must never overwrite a correct `false`).
...
- **`init()` is single-flight** (`_initFuture`). The old `if (_initialized) return;` sat in front of two awaits, so a burst of messages during startup could run `_win.initialize()` concurrently -- and that `CoRegisterClassObject`s a fixed GUID. A failure clears the cached future so a later message retries instead of the session staying dead.
- **Every branch logs to `hollow_debug.log`** via the top-level `notifLog()` (`network_api.logFromDart`, prefix `[HOLLOW-NOTIF]`). `debugPrint` is NOT sufficient -- it goes nowhere in release, and "no notification appeared" is only ever reported FROM release. Covered: backend ready, backend init failed, dropped-because-uninitialized, toast posted, toast failed, muted-conversation, and (in `event_provider`) suppressed-by-the-viewing-gate with the `visible=`/`focused=` values. The success line matters as much as the failures: if it appears and no toast is on screen, the app did its job and the OS suppressed it. See `feedback_notification_focus_two_sources` for the full triage ladder.
