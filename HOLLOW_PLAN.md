...
- [x] **At-rest file encryption (issue 78, SHIPPED 2026-09-11).** Every content file under the data root (`files/`, `audio_cache/`, `vault_cache/`, `shares/`, the wallpaper) is chunked AES-256-GCM (`node/at_rest.rs`, format `HFE1`: 32-byte header with a random per-file uid, 1 MiB chunks or the transport's chunk size, nonce = file nonce || index, AAD = uid || index || last flag); the per-file key lives in the SQLCipher table `file_keys` keyed by uid, so keys inherit the identity protection mode and deleting the row is the cryptographic erase (delete-for-me, caps, retention, share cancel all go through `at_rest::remove`). File names never change; a boot sweep (spawn_blocking, throttled, resumable, streaming) encrypts what an older version left in plaintext and the Storage Manager shows "Protecting your files (n of m)" then "Protected". Dart never opens a data-root file with dart:io: images via `AttachmentImage` on a custom `ImageProvider` (Flutter's ImageCache owns eviction), video and voice via a loopback HTTP server in Rust (`node/at_rest_server.rs`: 127.0.0.1, random port, per-launch token, Range, data-root confinement) because fvp and audioplayers need a URL, thumbnails and the Windows WAV transcode through ffmpeg pipes, Save-as = explicit decrypt-to-destination (`exportAttachmentTo`, toast says the copy is unprotected). `vault/` shards were already ciphertext; `.stream_*`/`.ws_recv_*` temps are transfer ciphertext; `temp/` (recorder, mic test) is emptied at boot; `hollow_debug.log` stays plaintext. Proof: 9 unit + 4 harness tests (honesty proof: the DM test failed against the old write path first), fleet journey `scripts/fleet_at_rest.ps1` (seed on the plaintext build, then upgrade: migration, render, fresh receive, voice over loopback, serve to a late joiner from an encrypted holder, export round trip). Residuals: mobile video/voice over the loopback verified by config only (Android `network_security_config.xml` 127.0.0.1, iOS `NSAllowsLocalNetworking`), a moov-at-end video cannot be re-thumbnailed through a pipe (the sender's poster is used), a voice recording in progress is plaintext in `temp/` until sent, `archive/loader.rs` still extracts archive attachments to the OS temp dir. Part 3 became ONE App Lock on both platforms (2026-09-11): the app password locks the UI (at launch, after an idle span, after 15 s in the background on a phone, on "Lock now"), the node stays alive, and Hollow starts on its own because the unlock secret is held by the OS keystore (Rust keychain on Windows/macOS, the app's secure storage everywhere), so every prompt runs with the keys in memory and a duress code fires the wide scopes from all of them; "Ask for the password before Hollow starts" is the advanced opt-in that keeps the key unreachable without the password at the cost of a local-only duress at that prompt. The strict "node stopped while locked" variant was rejected (keys out of memory while away is its only benefit and it turns every re-lock into a cold launch); the separate keychain-only mode left the UI. Memory `project_at_rest_file_encryption_plan`.
- [x] **Duress code and identity destruction (Part 2, 2026-09-11).** A second secret typed at the same prompt as the real one never unlocks, it destroys. Slot `identity.duress` (same HKEYV1 layout, own salt, fixed 42-byte plaintext carrying the scope and the friend flag; a dummy slot under a random key exists whenever password protection does, so neither the disk nor a stopwatch reveals whether a code is set); every typed secret derives Argon2id against BOTH slots before deciding; the duress code must differ from the password and `change_password` refuses the duress code. Three scopes: (a) this device, (b) plus a master-signed self-revocation (the ONLY device-list shape a tombstoned deliverer may publish, admitted only as the exact minimal diff against the stored list so revocation stays final against a modified client), (c) the whole identity via a master-signed `DestroyIdentity` (sibling Olm lane + plaintext twin to online siblings; the relay kill list for offline ones: an opaque blob parked per target device id, handed as the first frame after `auth_ok`, acked after the wipe or after a permanent rejection, persisted in snapshot v2 with 64 per issuer / 10 000 / 365 days fair-share caps; friends optionally told over the DM lane, their verified flag clears, a banner marks the conversation, and a later device list from that master raises an "identity reappeared" alert). Receiver rules: signature, own master, named targets, `issued_at` newer than this device's link stamp and than the last order applied in this process (never persisted: a stamp that outlives a failed wipe would let the next delivery be refused and acked). ONE wipe routine `destroy_data_root` (marker, keys zeroed and unlinked, keystore slot, session key, at-rest ring, store singleton, DB, content dirs; never waits on the network; Dart then clears App Lock, unregisters the push token bounded 2 s, relaunches to Welcome). Cold-launch duress (wrong password by definition) destroys LOCALLY only, so both duress cards explain that the wide scopes fire only when Hollow was already running (locked or not) and a code typed at launch destroys that device only; offline devices receive an identity-wide order when they next connect (relay hold up to a year, stated in Settings): nothing can be signed without the real password, and storing a device key in the slot was rejected as a device-impersonation key; a running node (mobile App Lock re-unlock) runs the full scope. Settings > Security gains the Duress code and account Danger zone cards on both platforms. Relay: `kill_list.h`, `kill_deposit`/`kill_ack`/`unregister_push_token`, plus a guard around the JSON dispatcher (a malformed frame from any authed client used to end the process). Proof: 9 unit + 6 harness tests (honesty proofs for the link-time refusal and the narrowed carve-out), relay codec + kill-list standalone tests, fleet `fleet_relay_restart.ps1` G5/G6.
...
- [x] **Large-server scaling Tiers 1–3 (2026-07-06).** Investigation proved the 50k wall is O(N) relay fan-out, NOT MLS (plaintext auto-downgrade REJECTED — see `reports/shipped/relay-and-sync/LARGE_SERVER_SCALING_2026.md`). Shipped same day: **T1** every MLS commit fan-out rides ONE `SendToRoom` via `broadcast_mls_commit()` (wire `epoch` guard — receivers at/past it skip instead of re-bootstrapping; join SendDirect 161/550/904 → 162/482/774, per-msg slope 2.5 → 2.0); **T2** CRDT ops flood the WebRTC mesh (data-channel type 0x04 `GossipCrdtOp`, op-newness bounds propagation, same author-permission ingest as relay ops, relay fallback when mesh isn't up — also killed the O(N²) receiver re-forward loop); **T3** ICE route class feeds `PeerScore.is_direct` (direct +0.15 / unknown +0.075 / TURN 0) so 300s rotation drifts toward directly-reachable peers. T4 (relay bandwidth sharding) remains the infra plan. Live 2-machine mesh-flood + route-bias check = manual pass pending.
...
- [ ] **Media viewer, albums and subtitles.** One viewer for images, GIFs and video, Telegram-style albums, and subtitle tracks. Full design in `reports/planned/voice-and-media/MEDIA_VIEWER_ALBUMS_SUBTITLES_PLAN.md`.
    - [X] Part A, shipped 2026-09-14: true fullscreen through a `hollow/window` method channel in the Windows runner, `window_manager` on macOS and Linux, a rebindable F11, and rotation unlocked only on the mobile media route.
    - [X] Part B phase 1, shipped 2026-09-14: one viewer route for images, GIFs and video (zoom, actual size, crisp pixels, rotate, walking the conversation's media through `list_media_for_context`, info panel, reply, react, save, copy, delete), plus the preview-line fix (`messagePreviewText`, no more raw tokens in conversation lists and notifications). Images open in the window, video keeps the OS fullscreen flow.
    - [ ] Part B phase 3, planned: eyedropper, annotate and send back, media tab, frame step, copy frame.
    - [X] Part C, shipped 2026-09-17: albums of up to ten files as ordinary messages sharing a signed `album` id (v3 canonical payload), a reorderable staging strip on every composer, one grouped mosaic bubble, one notification per album.
    - [ ] Part D, planned: subtitles (sidecar, embedded on desktop, cue editor, embed into the file).
...
- [X] **DONE 2026-09-10: self-hosting overhaul (issues #73, #57, #70 leftovers).** ONE `.env` compose: DuckDNS name by default (DNS-01, no port 80), bare VPS IP (Let's Encrypt IP certs, 6-day, GA since 2026-01), own domain; `certbot-init` gates the relay so a failed issuance stops `docker compose up` loudly; coturn opt-out via `COMPOSE_PROFILES=`; `deploy/harden-host.sh` = the production host setup; relay hot-reloads a renewed certificate (no restart); TURN URIs now derive from `--domain` (they were hardcoded to the official host, so TURN had never worked self-hosted); `/relay-status` advertises `turn`/`forwarder`/`version`. App: every invite stamps the sender's relay, a differing relay opens ONE "This server lives on another relay" dialog (never auto-switch), the join finishes after the restart; `No TURN server` chip, pre-dial refusal under Always relay calls, link-loss toasts name the cause; status.json banner only on the official relay. Guide `relay-uws/SELF_HOSTING.md`. Fleet `scripts/fleet_relay_switch.ps1` GREEN against a real Docker relay on the VM. Open: IP-cert mode and `OWN_CERT_DIR` untested (need public port 80), per-server multi-socket routing (research). Memory `project_self_hosting_overhaul_2026_09`.
- [X] **DONE 2026-09-17: UnifiedPush on Android (issue #75), the last self-hosting gap in push.** An Android user picks a distributor (ntfy or another) in Settings > Notifications > Push Delivery and the wake-ups stop going through Google; Firebase stays the default and iOS keeps APNs, which is the only way to wake a killed iPhone. The relay did NOT change: it already holds one opaque token per device, so a UnifiedPush endpoint replaces the Firebase token and Firebase registration no-ops while a distributor is active, one provider at a time. The sidecar sends RFC 8291 Web Push (encrypted to the device, so the push server sees only ciphertext where Firebase sees the sender id), refuses any endpoint that is not https on a public host (checked inside the socket's DNS lookup and again for IP literals), and no longer needs Firebase credentials at all, so a self-hosted relay runs the new `push` container with no Google account. Phone-verified both ways, DMs and channels, app swiped away. Doc `reports/shipped/relay-and-sync/UNIFIEDPUSH_PLAN.md`, memory `project_unifiedpush_android`. Open: the F-Droid build itself (#56) still needs a Firebase-free flavour, since the scanner rejects the library even unused.
...
- [ ] **Multi-relay CLIENT (designed 2026-09-12, not the mesh below).** One client holds a socket to SEVERAL trust domains at once (the official domain plus self-hosted relays); every server and friendship is pinned to the relay it lives on; identity rooms join everywhere; NO relay-to-relay traffic. This is how a self-hosted relay stops being an isolated network, and it is the answer to every "does Hollow have federation" question. The own-swarm mesh below scales ONE trust domain behind ONE relay entry and needs no pinning; the two coexist. Full design, phasing and open questions: `reports/planned/relay-and-sync/MULTI_RELAY_CLIENT_PLAN.md`.
...
> **Full design & decision report:** `reports/shipped/relay-and-sync/ANTI_CENSORSHIP_TRANSPORT_2026.md` (deep-research: 105 agents, 24 sources, 25 adversarially-verified claims, 2026-07). This section is the summary; the report is authoritative.
...
**Checklist:**
- [x] Research (2026-07): full field-status + embeddability sweep → `reports/shipped/relay-and-sync/ANTI_CENSORSHIP_TRANSPORT_2026.md`
