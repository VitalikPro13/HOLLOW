...
- [ ] **NAT-traversal hardening — router port-mapping via the `portmapper` crate (UPnP-IGD / NAT-PMP / PCP).** Research (2026-06-12) confirmed WebRTC's ICE agent already gives us ~90% of what Tailscale does for free — host/srflx(STUN)/prflx/relay(TURN) gathering, UDP hole-punching (simultaneous-open), ICE-TCP candidates — and our relay already serves all three TURN URIs (`turn:…:3478`, `turn:…:3478?transport=tcp`, `turns:…:5349`), so TURN-over-TCP and TURN-over-TLS are already in the ICE config (`ice_config_provider.dart`). The remaining gap libwebrtc does NOT cover is router port-mapping. Add the Rust `portmapper` crate (iroh/n0 team, MIT/Apache, does UPnP-IGD + NAT-PMP + PCP, maintains the mapping across IP changes): on node start / network change, request a mapping → get a public `ip:port` → advertise it to the peer via signaling so it's added as a remote ICE candidate. ICE's direct-over-relay candidate priority then picks the port-mapped path automatically whenever the router cooperated — no "use this instead of TURN" logic needed, TURN stays the fallback. **Highest-value target: the STUN-only paths** (`shareIceConfigProvider`/`streamIceConfigProvider` — Share, video streaming, large files) which have NO TURN fallback today, so a peer who fails STUN hole-punching currently can't participate at all; port-mapping gives them a second shot at a direct connection without opening TURN to bulk traffic. Caveats: only helps where the router speaks UPnP/NAT-PMP/PCP (home/consumer routers mostly do; corporate firewalls + CGNAT almost never — those still need TURN); mobile-on-cellular is moot (CGNAT, no mappable router) so it's mainly a desktop/home-WiFi win; the candidate-injection step is the real integration work (flutter_webrtc has no clean "add self-discovered host candidate" API — likely surfaced via signaling as a remote candidate). Skip the birthday-paradox symmetric-NAT port-prediction trick (significant engineering, only helps one-hard-one-easy NAT pairs, nothing for symmetric-vs-symmetric which always needs relay). *Optional later sub-task:* move/duplicate TLS-TURN onto port 443 (`turns:…:443?transport=tcp`) for firewall camouflage (5349 is a non-standard port strict corporate firewalls block; 443 is indistinguishable from HTTPS) — complication: coturn must coexist with the uWebSockets relay already owning 443 (SNI routing or a second IP). See session discussion 2026-06-12.
...
- [ ] **Decouple file persistence from auto-download (disk-space + ownership).** Today ALL files ≤34 MB auto-download to disk in DMs/channels (>34 MB uses Hollow Share, which is desktop-only → dead on mobile). Images stay auto-download (negligible: 50% WebP ≈ 97% smaller, looks identical). For NON-image files (video/docs/etc.): deliver + persist the SIGNED FileHeader immediately (sender/ts/sig/hash = bytes-not-megabytes, preserves the evidence/ownership story), but DON'T auto-fetch bytes — show a type-aware card ("📄 File", "🎬 Video", "🎵 Audio", "📎 caption") with a Download button; fetch bytes only on tap. Verify FileHeader commits to a content hash so the header alone is proof. Decide where bytes live for reliable on-tap fetch (likely erasure-coded vault by default so it survives both peers offline). Push notifications: type-label only (NO background byte fetch — too heavy). Defer video streaming/transcode as a separate epic. See session discussion 2026-06-09.
- [ ] **Storage management / cleanup tooling (needed regardless of the above).** No way today to see or reclaim disk used by downloaded files. Add a "Manage Storage" UI: per-conversation + total size breakdown, "clear cached file bytes" (keep signed headers so messages/cards remain, just drop the heavy bytes — re-fetchable), and an LRU cap on `~/.hollow/files/` (mirror the existing 1 GB `vault_cache` cap). Highest-relief / lowest-risk slice of the file rethink — do this first.

- [ ] **Multi-device identity & sync (major epic).** The 24-word mnemonic is the *identity*, not the *data* — importing it on a new device gives an empty DB (no central server to re-hydrate from). Symptoms: mobile import is empty while desktop is full; a friend request from the empty mobile device renders on desktop as "your own friend sent you a friend request"; two installs of one identity corrupt each other's crypto state. **Decided model (Signal-style):** one master identity (shared, profile-attached signed device list) + per-device Olm/MLS sub-sessions so both devices can be online at once; QR linking (desktop shows, phone scans) with a one-time symmetric key streaming a standalone-encrypted DB snapshot through the relay; sender-side fan-out for new messages; backfill from your other device first, conversation peer (signature-verified) as fallback; master-signed revocation with monotonic version (replay protection); and a **Sync Health panel** in Settings (per-device DB health comparison, manual Sync button, device management/removal). **Deferred — implement later, after current DM/file fixes.** Full design (decisions, flows, scope, sequencing, rejected alternatives) in `reports/MULTI_DEVICE_SYNC_PLAN.md`.
