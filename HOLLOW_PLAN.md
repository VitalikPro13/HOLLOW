...
- [ ] **Video preview in chats** — thumbnail-first, click-to-play, on-demand vault reconstruction. Reuses existing `vault_upload_file` / `vault_download_file` / `vault_cache/` infrastructure. Architecture decision: in 6+ servers the sender extracts a small JPEG thumbnail (first frame) which goes through the **image** path (full P2P streamed → instant scroll for everyone), and the actual video bytes go through the **vault** path only (no full-file P2P streaming, no `files/` storage on receivers). Receivers see the thumbnail immediately and only pay the reconstruction cost on click. Universal cache: even the uploader plays from `vault_cache/` (same code path as receivers — no special-case sender logic). No upload pipeline changes needed — the existing `store_full_file = is_image || member_count < 6 || dm` gate in `swarm.rs:4698` already routes correctly.
  - [ ] **Wire format: add `videoThumbnailFileId: Option<String>` to `FileHeader` envelope variant.** When non-null, the message has both a thumbnail (full-replication image, regular file) AND a vault video (referenced via the existing `file_id` → vault `content_id` lookup). Use `#[serde(default)]` for backward compat — old clients ignore the field
  - [ ] **Rust: thumbnail extraction.** Add `extract_video_thumbnail(file_path: String) -> Result<Vec<u8>, String>` FFI in `api/network.rs`. Implementation: invoke `ffmpeg-next` crate (or `video_thumbnail` Dart-side via flutter platform channels — decide based on what's available on Windows/macOS/Linux without C++ build pain). Returns JPEG bytes of the first frame at ~480p. Run in `tokio::task::spawn_blocking` to avoid blocking the runtime
  - [ ] **Dart: detect video files in `file_transfer_provider.sendFile`.** Helper `_isVideoExtension(filePath)` matches `mp4|webm|mov|mkv|avi|m4v`. When true: (a) call new `extractVideoThumbnail()` FFI to get JPEG bytes, (b) write thumbnail bytes to a temp file, (c) call existing `network_api.sendFile()` for the **thumbnail** (not the video — this lands the thumbnail in `files/` via the image path), (d) call `vaultUploadFile()` for the **video** (member_count >= 6 only — for <6 servers, video also goes through P2P file path like today's behavior), (e) link them via the new `videoThumbnailFileId` wire field
  - [ ] **Dart: video player widget.** New `VideoMessageBubble` widget. Renders the thumbnail by default (reuses existing image rendering — same `files/` path). Overlay: ▶ play button + duration + filesize + format. On tap: call `vaultDownloadFile(serverId, contentId)`. If FFI returns a path immediately → cache hit, play instantly. If returns empty string → async reconstruction in flight, show progress bar driven by `VaultDownloadProgress` events, then play when `VaultDownloadComplete` arrives with the disk path
  - [ ] **Dart: inline player.** Use `video_player` package (Flutter team-maintained, plays from local file paths). Wrap in `RepaintBoundary`. Standard play/pause/scrub controls + close button. Player auto-pauses when scrolled out of viewport (use `VisibilityDetector`)
  - [ ] **Save button (works for ALL non-image vault files, not just videos)** — new `Save` action in the file message context menu / hover bar. On click: (1) ensure file is in `vault_cache/` (call `vault_download_file` if missing), (2) `file_picker` package shows native Save As dialog with default filename = original `file_name` from manifest, (3) `File.copy(vault_cache_path, user_chosen_path)`. The vault_cache copy persists for fast re-access. **Replaces the current "click → instant download to fixed location" UX for vault files** — better because the user picks where, AND the file is now in the LRU hot cache for free
  - [ ] **`vault_cache/` cap consideration.** Current: 1GB hard cap, LRU eviction in `pipeline.rs:284`. For video use case: bump to 2GB? Or make it user-configurable in storage settings dialog? Decide during implementation. Cache eviction already runs every 30 min via the rebalancer (`swarm.rs:6581`) — no new wiring needed
  - [ ] **Thumbnail fallback.** If `extract_video_thumbnail` fails (corrupt video, codec not supported, no video stream), use a generic black-with-▶-icon placeholder. Never block the upload on thumbnail failure — the video still uploads, the user still sees a playable card
  - [ ] **DMs and <6 servers** — videos go through normal P2P file path (no thumbnail extraction needed, the video itself is in `files/` and plays inline directly). Only 6+ servers need the thumbnail+vault dance. Branch on `memberCount` in the Dart side
  - [ ] **Test plan.** (a) Send 30MB MP4 in 2-person DM → plays inline directly, no vault, (b) Send 30MB MP4 in 8-person server → thumbnail in scrollback, click play → vault reconstruction → plays from cache, (c) Send same video again → cache hit, instant play, (d) Click Save on a non-image file → File Explorer opens, chosen path receives the file, (e) Cache eviction: fill cache past 1GB → oldest videos evict cleanly without breaking the player widget that's currently displaying one
