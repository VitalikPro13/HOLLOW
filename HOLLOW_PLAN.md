...
- [ ] **Media viewer, albums and subtitles.** One viewer for images, GIFs and video, Telegram-style albums, and subtitle tracks. Full design in `reports/planned/voice-and-media/MEDIA_VIEWER_ALBUMS_SUBTITLES_PLAN.md`.
    - [ ] Part A, in progress: true fullscreen through a `hollow/window` method channel in the Windows runner, `window_manager` on macOS and Linux, a rebindable F11, and rotation unlocked only on the mobile media route.
    - [ ] Parts B, C and D, planned: the viewer itself (zoom, pan, navigation, pixel-exact mode, info panel), albums as N messages sharing a signed `album` id, and subtitles (sidecar, embedded on desktop, cue editor, embed into the file).
