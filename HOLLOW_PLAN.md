...
- [ ] **Wayland-native global PTT/mute hotkeys via the XDG GlobalShortcuts portal** (follow-up to issue #38 hotkeys). The X11 GetAsyncKeyState/XQueryKeymap poll covers X11 sessions and XWayland-focused apps (incl. Proton/Wine games) but is blind while a native Wayland window has focus. Proper fix = org.freedesktop.portal.GlobalShortcuts (KDE 5.25+, GNOME 48+): D-Bus register + Activated/Deactivated signals (press AND release, so hold-PTT works), compositor consent dialog. Same portal family as the screen-capture picker. macOS global hotkeys (CGEventTap + Accessibility permission) are the sibling follow-up. **WINDOWS PTT IS ALSO BROKEN (2026-08-03, field-tested twice):** mute/deafen toggle hotkeys work through the exact same GetAsyncKeyState poller + edge routing, but holding the PTT key (Ctrl+Space) never opens the transmit gate and never flips the PTT-aware mic icon — even after the round-3 fix that re-reads the voice-hotkey settings at first call entry (the bootstrap-not-build stale-load theory alone did NOT explain it). Debug from the `[HOLLOW-HOTKEY]` console lines (backend-start binding list, per-edge prints, `VC PTT transmit=` prints): they bisect whether the PTT binding registers, the edge fires, or the gate/`pttStateProvider` write is the dead link. All the plumbing (transmit gate in both call notifiers, release-delay timer, PTT mic visuals, settings card) is in place — only the chain break needs finding.
