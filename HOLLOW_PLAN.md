...
- [ ] **Sweep the MLS-first-else-plaintext fan-outs (found 2026-08-29 by the pending-join fleet journey).** `handle_delete_server` sent the plaintext `CrdtOpBroadcast` twin only when the OWNER's MLS send failed, AND computed the fallback's targets from `state.members` AFTER `apply_op(ServerDeleted)` had cleared them, so the fallback was dead code on every path: a member with no MLS leaf (a just-admitted parked joiner, or anyone at a stale epoch) kept a ghost server after a delete. Fixed there (targets captured BEFORE the apply, twin sent unconditionally, guarded by `server_deleted_reaches_a_parked_member_with_no_mls_leaf`). The same `let mls_ok = ...` shape remains at `social.rs:995`, `:1182`, `swarm.rs:10306`, `vault_ops.rs:444`, `:521`, `voice_handler.rs:407`, `:1034`, `file_handler.rs:1828` (and the join arm's `MemberAdded` at `swarm.rs:9814`, covered by the ring resolution's `op_json`). Each needs checking for BOTH the guard and what the fallback is aimed at after the op mutates state; each fix wants a harness test that makes the MLS leg unreadable on the receiver (`set_broadcast_deaf` / no leaf). Memory `feedback_mls_first_fallback_dead_targets`. **Rung 2 of pending joins, same follow-up slot: the carried KeyPackage.** Rung 1 parks the CRDT admission only, so a joiner admitted while away holds the server (name, channels, members) but cannot read anything said before its first co-presence, and shows "waiting for a member to finish setup" until then. Rung 2 puts the joiner's MLS KeyPackage INTO the parked ring copy so the admitting member also commits the add and the buffered `MlsWelcome` lands on the joiner's next boot; from then on ring catch-up decrypts what was said after admission (never before: forward secrecy). Prerequisite first, and it is a real bug on its own: `generate_key_package` writes the private half only to OpenMLS's in-RAM `MemoryStorage` and `persist_mls_state` is NOT called after minting, so a restart before any incidental persist makes every Welcome for that KP fail with `NoMatchingKeyPackage`. Fix = persist after mint (and delete the bundle on discard). Then consume-once: the resolution in the ring plus the already-member skip already prevent a second commit against the same KP; the joiner keeps ONE KP per pending row for every re-deposit and live re-request (OpenMLS accepts a re-add of the same KP after the old leaf is removed, and the joiner still holds its private half until it processes a Welcome). Harness: the zero-overlap test extended with a channel message sent by the admitting member BEFORE the joiner returns, decrypted by the joiner ALONE. Memory `project_pending_server_joins`. **Second item for the same sweep, found by CI on 2026-08-29: the join bootstrap sends two KeyPackages and reacts to its own re-add.** A joiner sends a KeyPackage at `ServerJoined` (to the responder that served it, `swarm.rs` "Sent bootstrap KeyPackage to join responder") and AGAIN when any MLS frame reaches it before its Welcome (the owner's MLS-first `MemberAdded` broadcast always does; "triggered by message", throttled 60 s but the join-time send does not record the throttle). Two copies that straddle a 2 s batch tick become a remove + re-add; the joiner reads the removal commit as an eviction ("Commit EVICTED us, dropping inactive group") and sends a third KeyPackage to the owner, which is another remove + re-add. A clean three-member join reached epoch 5 on Linux CI. Two gaps to close together: (a) a non-owner responder that holds no group SKIPS a KeyPackage ("No MLS group and not owner, skipping") instead of the join-time send targeting the bootstrap target, which is why the second send is currently a needed crutch (recording the throttle at the join-time send alone made a joiner whose responder was group-less never get a leaf; reverted); (b) a device whose leaf is being replaced by a remove + re-add in one batch must not treat the removal commit as an eviction (the Welcome follows in the same tick). Guard with a harness test that asserts a three-member live join lands at exactly the minimal epoch count on the owner. Test-side, the stale-epoch tests now wait for a quiet group before staging their heal.

- [ ] **Twitch identity and gate are honest-client only (found 2026-08-28, fix AFTER the artist shop epic, same machinery).** `twitch::validate_proof` trusts the JOINER-produced JSON (`followed_at`, `is_subscribed`, freshness) and never contacts Twitch; the profile `twitch_username` and the member `TwitchUsernameChanged` op are self-declared (the op is even stamped from the joiner proof). A modified client edits two fields to pass any Twitch gate, and can wear any streamer handle in the purple twitch.tv chip on its profile card, which reads as a connected account. Fix = a Hollow verifier endpoint that checks the OAuth token itself and BLIND-signs `(master_peer_id || twitch_user_id)` and `(master_peer_id || channel_id || follow_since || sub)` with RFC 9474 (per-channel issuing keys, root-signed, offline verification by owners and viewers), exactly the credential design in `reports/ARTIST_SHOP_DESIGN.md`; the endpoint sees the Twitch account (it must) but never the peer. Until then the chip is a claim, not a verification; consider labelling it so. **Rung 1 limit to lift HERE, after the shop (2026-08-29):** parked server joins strip `twitch_proof_json` from the `~join` ring copy, because that JSON carries `twitch_user_id` + `twitch_username` and the ring is readable by any socket holding the invite (peer ids alone are accepted exposure; a Twitch identity tied to a peer id is not). So a parked join on a Twitch-gated server stays pending until co-presence, when the live re-request carries the proof; the member side returns silently on a parked copy for any Twitch-gated server. When this item moves the gate to blind credentials (an unlinkable proof of follow/sub, no username), that credential CAN ride the ring copy and Twitch-gated servers get zero-overlap joins like everyone else. Same machinery as the shop, same epic, do it right after.
