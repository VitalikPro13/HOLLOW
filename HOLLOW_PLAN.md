...
- [x] **Archive tab — personal data viewer + signed `.hollow-archive` export/import (part of "The RAT Files" protocol)**
  ...
  - [ ] **TODO: Preserve original message signature through edits.** Currently `edit_channel_message()`/`edit_dm_message()` in `storage/messages.rs` overwrite the main-row `signature`/`public_key` columns with the edit's signature, destroying the original send signature. The `message_edits` table only stores the *new* edit's signature, not the previous one. This means the archive edit history timeline cannot verify the original (pre-edit) message text — the first entry's shield shows grey. Fix: store the original `signature`/`public_key`/`timestamp` in the first `message_edits` INSERT (as separate columns like `prev_signature`/`prev_public_key`/`prev_timestamp`), or add a dedicated `original_signature`/`original_public_key` column pair to the messages table that never gets overwritten. Both the archive exporter (`ArchiveEdit` struct) and the `EditHistoryIndicator` widget are already wired to accept and display per-edit signatures — just needs the data preserved.
...
- [ ] Fix the icon outline size on the server strip (Gear is bigger than Downloads/Archive)
