...
- [ ] Get Linux out of Experimental phase, please
  - [ ] **Linux screen-share audio anti-echo + per-app capture.** Screen-share audio send+receive SHIPPED 2026-07-04 (PulseAudio monitor capture → 0x03 data channel; 2-machine confirmed), but v1 captures the WHOLE system mix: an entire-screen share re-captures the call audio (remote hears themselves), and a window share still sends full system audio. Fix for both: a `pa_stream_set_monitor_stream` INCLUDE-set mixer in the exe (the PulseAudio analog of Windows `MultiProcessCapturer`) — capture every sink-input EXCEPT Hollow's own (by `application.process.id`) for entire-screen anti-echo, or ONLY the shared app's sink-inputs for per-app. Needs dynamic subscribe (sink-inputs come/go mid-share) + PipeWire-pulse compat verification. See memory `project_linux_screen_audio`.
