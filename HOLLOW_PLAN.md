...
- [ ] Server banners
- [ ] GIF picker (support on our side and let users to bring their own API key too - from what I see, Klipy feels like the best option)
- [ ] Other types of channels (like forum channels), threads, categories
- [ ] Add ability to change input/output on mobile
- [ ] Update the Classic mode + make sure that the panel with tabs in Server settings is properly designed like a carousel rather thhan static thing
