...
- [x] **Relay per-IP daily byte budget (anti-drain backstop, 10 GB/day).** Done 2026-07-05, deployed + live-verified (guest WS test: 100 KB frame counted exactly). Relay: `bytes_today`/`budget_day` on `IpState` (keyed by `ip_limit_key` — v4 addr / v6 /64), cached `IpState*` on PerSocketData (zero-lookup per frame), counts BINARY both directions (inbound `.message` + outbound `send_to_peer`, attributed to the RECIPIENT so download-drains count), lazy UTC-day rollover, enforcement = explicit `1008 "bandwidth_limit"` at inbound binary AND text entry (text catches topic_catchup leeches; never closes mid-fan-out), entries with bytes survive disconnect (reconnect can't reset), stale-day purge on the 300s sweep. New `get_bandwidth` WS command → `bandwidth_status {used, budget, reset_in_secs}` — reply rides the exact counted connection (an HTTP poll could resolve the other address family). Client: `request_relay_bandwidth()` FFI → `NetworkEvent::BandwidthStatus`; close-reason now read → `BandwidthLimited` event + error toast. UI: "Your daily usage" StatBar + "Resets in" countdown in the desktop Home relay card AND a new mobile Settings "Relay Server" card below Your Stats (shared `StatBar` component, both polls visibility-gated autoDispose, 30s). See memory `project_relay_bandwidth_enforcement`.
...
- [ ] Add ability to change input/output on mobile
- [ ] Update the v2 grammar port to web viewer

- [ ] **Wayland screen share — window sharing + libwebrtc null-safety (issue #30 follow-up, 0.9.0 shipped a GATE not a cure).** What 0.9.0 does: `flutter_screen_capture.cc` predicts what the session can actually produce (Wayland → screens via portal/PipeWire; otherwise probe `XOpenDisplay`) and never asks libwebrtc for the impossible list, with `DesktopCaptureSupport` mirroring it in Dart. X11 keeps individual windows, Wayland stops crashing. Two things remain. **(a) Wrapper null-safety** — `third_party/libwebrtc/hollow-wayland-desktop-capture.patch` is written and applies cleanly to `be9743f`, but is NOT in the vendored `libwebrtc.so`/`.dll`: `RTCDesktopMediaListImpl`'s ctor and `RTCDesktopCapturerImpl`'s ctor + `Start()` still call straight into a null capturer, so any session shape our gate mispredicts is a SIGSEGV rather than an empty list. Apply at the next libwebrtc rebuild (BUILDING.md step 2, VM `~/libwebrtc-build`), fold into `hollow-screencast.patch`, rebuild BOTH binaries together. **(b) Window sharing on Wayland** — needs `set_allow_pipewire(true)` for `kWindow` as well, but the flag alone is worse UX than no windows: every media list that touches the portal pops an xdg-desktop-portal dialog at picker-open time, and Wayland already costs two prompts (picker open, then share start). The real shape is a portal-first picker — on Wayland show ONE "Share screen" entry with no thumbnail grid, let the portal pick screen-or-window, and carry a restore token so re-shares stop re-prompting. `feedback_wayland_window_capture_sigsegv`.
...
- [ ] Media gossip forwarding — extend the voice gossip tree to CAMERAS and SCREEN SHARE (verified 2026-07-13: only AUDIO forwards today). Voice channels already flip mesh→gossip at 6+ participants (`voice_handler.rs` thresholds 6 up / 4 down, ≤12 neighbors) and forward received AUDIO tracks neighbor-to-neighbor, but `voice_channel_service.dart` onTrack explicitly skips video ("audio only for now") — so in gossip mode camera feeds only reach DIRECT neighbors, and screen share is a separate per-viewer PC service hard-capped at 5 outgoing viewers with no forwarding at all. Also missing: multi-hop originator attribution (track dedup keys on the immediate forwarder's peerId, not the original speaker — must be signaled before >1-hop trees). Needed for: large-VC webcams, screen share beyond 5 viewers, and the conference broadcast/streaming mode.
