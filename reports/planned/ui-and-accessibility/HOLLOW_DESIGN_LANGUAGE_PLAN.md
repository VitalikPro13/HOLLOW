...
**Status:** IN PROGRESS, sessions 1 to 8 done 2026-09-19 (decisions applied, sweeps 3c to 9 done); session 9 (2026-09-23) wrote the screen standards and mocked up Home; sessions 10 and 11 finished Home; session 12 (2026-09-24) redesigned the chat screen on desktop and phone; session 13 (the same day) did the animation pass; session 14 (the same day) audited the dock and header and mocked up their redesign (approved); session 15 (the same day) built it; sessions 16 to 18 (the same day) rebuilt Settings, then Server settings, as places. Direction agreed 2026-09-14, research the same day.
...
- **Session 10, Home on the phone (2026-09-24).** The mobile Chats tab is the desktop inbox now, verified on the mini's iOS Simulators (`fleet/home_mobile.json`, with `fleet/home_mobile_before.json` as the same journey on the old tab; pairs in `build/mhome/cmp_*.png`).
  - **Shared, not copied:** `HomeActions` (desktop dialogs vs phone routes and sheets: `touch`, `installsUpdates`, add friend, add server, edit profile), `HomeGreeting`, `homeIsFirstRun` / `homeShowsSetup`, `HomeConversation` + `homeDmConversations` / `homeMentionConversations`, `HomeFilters`, `homeNothingToShow`. `ConversationRow(touch: true)` is the phone row (48 px avatar, type one step up, full bleed, long press). `showNewMessageDialog` takes `onOpen` / `onAddFriend`.
  - **What the phone adds:** servers stay in the list (a phone has no dock), ranked with the DMs, unread first; the Mentions chip lists the channels that mentioned you, and a server row carries the red `@` count, so a mention is never shown twice in All. Saved messages and parked joins stay pinned. No update row (the stores update phones). New message is an icon in the title row, since the nav bar's + already carries the accent. The "Hollow" wordmark in the accent and its shimmer line are gone (gradient 21 to 20).
  - **Caught by the renders:** the full six-step checklist filled a phone's first screen and hid the conversations (the probe could not even find b's DM), so on a phone with any conversation it shows only the next step plus "Show all steps" (`HomeSetupChecklist(compact:)`). The New message dialog's friend list inherited the iPhone safe-area insets as padding (a ListView default), leaving a band above and below each row. The chips and the attention buttons overflowed at 2x text (`Wrap` now).
...
- **Session 12, the chat screen (2026-09-24).** Audit first (two agents, desktop and phone, against 5.3), then a clickable mockup, https://claude.ai/artifact/7Yd3dTqcNrAQe3ZeUyfe72 (DM, channel, compact, name colours, phone DM, phone long press), then the build. Vitalik's verdicts: the DM panel to the right like the member panel; names coloured by person with your own in the accent (the teal strip on your messages goes); the colours contrast-checked per theme; the composer folded; compact display in this pass; member panel names neutral; the hover highlight rebuilt so it moves nothing and survives a scroll. His rule for the build: "the real UI needs to carry established rules and proper layout without stupid gaps".
  - **One of each** (design language 4.5): `MessageRow` (the two bubbles are adapters now), `ChatHeaderBar`, `ChatComposerRow`, `ChatSearchResultRow`, `showExpressionPicker` + `showExpressionSheet`, `DmProfilePanel`, `HollowIconButton` (components), `HollowShadows.float` (theme), `bodyTouch` 16 (theme), `rowHover` (theme), `messageDisplayProvider` (cozy / compact, Appearance on both platforms, loaded in `_bootstrap`).
  - **Hover, the root cause:** the highlight and the bar were Overlay entries placed at the row's position at the moment of hover, so a scroll left them over the wrong message (the panes papered over it by dismissing on every scroll tick). Now the row paints its own highlight behind the content and the bar is a `CompositedTransformFollower` on the row's `LayerLink`, clipped to the list; the dismiss-on-scroll listeners are gone from the DM, channel, guest and archive panes. Pinned by `test/widget/message_hover_test.dart`. The bar is 3 quick reactions, add reaction, reply, edit (own), More; More is the right-click menu, Delete last.
  - **Names:** `nameColorFor(master, hollow)` (FNV-1a so web and native agree; skips 35 degrees around the accent, whatever hue the user picked; 7:1 dark, 5:1 light on all five surfaces). `test/name_colors_test.dart`, 26 checks over six accent hues. Row hover is half a step to `elevated` so a card in a hovered row still shows.
  - **Status by exception:** Encrypted and Synced are silent; Offline, alone, syncing ("Syncing 12 of 40"), retrying and failed (with a ghost Retry) speak. Encryption moved into the DM panel as the one action, Verify.
  - **Composer:** `+`, the field with one smiley inside, the mic that becomes Send. The field's focus is quiet, and **`HollowTextField` lost its focus glow everywhere** (glow is on the forbidden list). The picker's inner "Emoji" chip is "Standard" now, so the tab and the chip do not repeat each other. On a phone the attach sheet is Photo or video and File (GIF and Sticker moved into the smiley).
  - **Bugs found and fixed:** clicking a reaction on desktop dropped the composer's focus (Flutter's desktop tap-outside default), so the next keystrokes went nowhere; reacting now hands focus back. The DM showed "No messages yet" while its history loaded, and a failed load left a channel blank forever: `loadHistory` returns whether it worked, and both panes have a "These messages didn't load" state with Try again. A voice channel's chat said `#lounge`. The phone's edit field was re-seeded on every rebuild, so a message arriving mid-edit wiped the edit. Phone reply quotes did nothing on tap, DMs never highlighted a jump target, and Message in the phone profile sheet opened a second copy of the chat you were in.
  - **Phone:** 44 px targets in the header, composer, sheets and edit view; message text `bodyTouch` 16; the long-press sheet in sentence case, grouped like the desktop menu, Delete alone at the end; the profile sheet is one primary plus an icon strip (nickname, verify, More with Copy user ID, Block, Report), the peer id and the at-rest Block/Report gone.
  - **Cards and small things:** no accent strips on link, invite or reply bars; invite titles sentence case; file and sticker-pack icons grey at 24; reactions are `HollowChip`s; the New line is the accent with a `HollowBadge`; dates are words without the current year; search rows are shared, and in the person's colour.
  - The member panel's default width is 280 now (5.2 says 280 to 300), shared with the DM panel. The server settings tab strip scrolls sooner as a result; `home_redesign.json` and `chat_redesign.json` scroll it before tapping Danger.
  - Verified with `fleet/chat_redesign.json` (desktop, two throwaway peers: befriend, DM with reply and reaction, hover bar, More menu, composer, picker on all three tabs, panel menu, light, a server channel with a mention, compact) and `fleet/chat_mobile.json` (the mini's simulators). The older phone scenarios target the composer as `field` and send with "Send message" now.
  - Guard baselines: font-size 541 to 495, material-colors 179 to 176, color-literal 129 to 126, radius 102 to 93, edge-insets 213 to 191, sized-box-gap 158 to 146, gradient 19 to 17.
  - **Follow-ups from Vitalik's review (same day, done):** the DM panel's avatar scales with the banner (72 at the default 280, 48 to 88), the icon strip moves under the name when avatar and strip no longer fit side by side, and Verify stacks under its text below a 220 px content width; the panel toggle is `circleUser` (the old `panelRight` read as the split view's `columns`) and sits LAST in the header, after split view, and the channel header's members toggle moved last to match; the hover bar is centred on its row again (straddling the top edge made it hard to reach from a one-line row). Scenario `fleet/chat_panel_fixes.json` (drags the panel narrow, to the minimum and the maximum); pair `build/cmp_chat_panel_fixes.png`.
  - **Left for later:** the channel list's selected row still has its gradient (`selection_shimmer.dart`); the mobile DM has no search (parity gap, the storage call exists); the staged-file remove X is small on a phone; the voice pane's own chrome and the call surfaces were not part of this pass; `colorFromId` (avatar colours) still uses `String.hashCode`, which the web computes differently.

- **Session 13, the animation pass (2026-09-24).** Audit first (every animation, desktop and phone, against 3.8), Vitalik agreed with every point ("overanimating everything is still a bad idea"), then the whole pass. The rules are rewritten in `HOLLOW_DESIGN_LANGUAGE.md` 3.8 and the skill.
  - **Tokens:** `HollowCurves.spring` (elasticOut) is deleted; `HollowDurations.exit` 100 ms is new; `HollowMotion.rise` 8 and `popoverScale` 0.96. Durations are read when an animation starts, never cached in `initState`, so a live Reduce motion change applies.
  - **Bugs:** every button and pressable released on the elastic curve (it held, then snapped); toggles overshot past the track and wobbled; context menus scaled from the SCREEN's top-left corner (the transition wraps the full-screen host), so a menu on the right slid in diagonally by about 40 px, now scaled around the click point; the profile card always grew from its bottom centre even when it opened downward.
  - **Pickers:** the emoji, GIF and sticker panels no longer scale from 0.94 (the far corner travelled 25 px and read as a stretchy slide): they fade and rise 8 px (`PopupAnimator(rise: true)`). The reaction picker takes the same corners and shadow. **On the phone the picker takes the keyboard's place** (`mobile/mobile_keyboard_panel.dart`): same height, composer never moves, the smiley becomes a keyboard icon, emoji insert and keep it open; `showExpressionSheet` is gone.
  - **One popover motion:** menus, pickers, profile card, folders, downloads at 150 in / 100 out (were 120, 140, 180, 180 at three scales); dialogs leave in about 150 ms; toasts rise 8 px, notification cards step 8 px in (were 320), the incoming call card drops 8 px (was its full height). Menu and popup shadows are `HollowShadows.float` (big-shadow 15 to 11), the profile card lost its accent border.
  - **Instant:** switching a conversation, channel, server or tab (the dock's nested cross-fades also rebuilt the ambient background on every switch), and the member, DM profile, channel sidebar, help and call panels. The 2.5 s startup choreography (typewriter header, staggered member list, clipped sidebar) is one 250 ms fade. The selected-row shimmer is deleted (flat accent-muted fill; gradient 17 to 16) and its 30 fps ticker with it. The new-server bounce is a fade and scale.
  - **Phone:** iOS edge swipe back with the page below moving 30 % (`mobile_page_route.dart`, Android keeps the system gesture), sheets on the tokens and under Reduce motion, tab and archive switches instant, the stagger in archive message actions gone.
  - **Smaller:** colours no longer lerp from `Colors.transparent` (dock indicator, reorder gaps, split focus border and divider); the typing bar pushed the list when it appeared: it is now `TypingIndicatorHost`, a small floating label on the seam between the list and the composer, lined up with the message text, reserving nothing and fading in only while someone types (a reserved 24 px strip was tried first and rejected: "I hate it"), desktop and phone; the unread pill fades; the annotate label floats instead of widening the title-bar button; about 25 Reduce motion gaps closed; the message proof dialog's 600 ms stagger is gone.
  - Tests: `test/widget/popover_motion_test.dart` (menu scaled around its anchor, a rising panel keeps its size and travels 8 px at most, someone typing moves neither the list nor the composer), `test/screenshots/typing_label_screenshot_test.dart` (the label on the real composer, both themes), `mobile_back_swipe_test.dart` (8), `mobile_keyboard_panel_test.dart` (4). Desktop renders via `fleet/chat_redesign.json` and `fleet/chat_panel_fixes.json`.
  - **Not verified yet:** the phone half on the iOS Simulator (the mini was off). **Probe trap:** a fleet peer's typing notice cannot be shot. The send is throttled to one per 3 s (a send just before stamps it), and a second `enter_text` into the composer is cleared before the next frame, so only the first ever sends; both peers' `hollow_debug.log` show `[HOLLOW-TYPING]` sent and received, so delivery works (Vitalik confirms on real clients).

- **Session 14, the dock and header audit and mockup (2026-09-24).** Nothing built yet. The pass is bigger than "polish only" (phase 2 step 3 below): four of its findings are structural, so it went audit, mockup, plan. Vitalik agreed with every audit point ("I think you're right on every point") and asked for the build to wait for its own session(s).
  - **Evidence.** `fleet/dock_header.json` (three throwaway peers: first run, two friend requests, two friends with an unread DM, three servers with a mention and a voice room, hovers, light, in a voice room; cleanup deletes the servers). Shots `build/fleet_out/a/k-01` to `k-12`. Two code audits (dock: `bottom_bar.dart`, `user_bar.dart`; header: `window_title_bar.dart`, `friends_bar.dart`, `system_status_banner.dart`).
  - **Mockup:** https://claude.ai/artifact/UhNWSo9vPMrTqAezH82cMR (Design canvas: the proposed desktop, clickable, with a selection-style and Home-mark tweak; today's real renders beside it; every dock state; every header state). **APPROVED in full by Vitalik the same day** ("I agree on the whole thing"; "simple UI that's understandable", no slop feel). His picks: the selection mark is the thin 2 px teal bar on the dock's top edge (the phone's mark), and the Home tile is the teal Hollow logo, the H with the padlock and keyhole.
  - **Broken today (fix whatever the verdict):** (1) in a voice room, Dock mode loses the call on Home or a DM: no mute, no leave, no "connected" (`VoiceChannelPanel` lives only at the foot of the server's channel sidebar); (2) the dock never shows your connection status (its identity block is a hand-built copy of `UserBar` without the dot); (3) a friend chip's unread badge covers the avatar's initials; (4) once the server row scrolls, its viewport (38 px) clips every badge's top and hides the active bar; (5) folders sum unread but drop mentions, and a selected folder hides its count; (6) the add-friend button opens the Friends tab even while its badge counts requests; (7) hovering a server paints the same bar as the active one; (8) a friend with no messages shows "Jan 1, 2000" on Home (Home's bug, found here).
  - **The proposal (the four structural decisions):** (A) **places vs tools:** every `ShellTab` (Conferences, Public channels, Share, Archive, Shop) moves to ONE group in the dock beside the servers; the header keeps only people; Saved messages becomes a conversation pinned at the top of Home (the phone already pins it); Help sits with Downloads and Settings as the dock's tools; the recovery-phrase key leaves the dock (Home's setup step owns it). (B) **one header strip in Dock mode:** the 32 px title bar folds into the 44 px friends bar (window controls, annotate and the zoom readout at its right end, the empty middle drags the window). Constraint: the title bar sits above the Navigator and outside `UiScale` on purpose (the zoom readout must stay legible at any scale), so the controls stay UNSCALED in an overlay at the strip's right end and the friends bar reserves their width divided by the scale. macOS's traffic lights need checking on the mini; if they cannot centre in 44 px, macOS keeps the thin bar. Classic keeps its title bar. (C) **voice in the dock's identity block:** your status dot, then while in a room its name and mute, deafen, leave, on every screen; the server holding the call carries a small green speaker; the sidebar's `VoiceChannelPanel` goes in Dock mode. (D) **one selection mark:** Home, servers, folders and places share ONE mark, proposed as the 2 px accent bar on the dock's top edge (the phone nav bar's pattern); Home becomes the Hollow mark on an `elevated` tile instead of a solid accent block; servers anchor straight after Home (no centring), Add ends the list.
  - The polish list and the verification plan lived in `tmp3.txt`; session 15 built all of it (below) and folded what is left into its entry.

- **Session 15, the dock and header built (2026-09-24).** Everything in session 14's plan, in one session, on desktop; the phone needed nothing (its tab bar already had the mark, Saved is pinned, the voice pill exists).
  - **The eight bugs:** voice now rides the dock's identity on every screen (`VoiceQuickControls`: mute, deafen, "Disconnect", one `leaveVoiceRoom` that toasts a failure; the sidebar's `VoiceChannelPanel` stays Classic only); the dock shows your connection dot (`DockIdentity`, from `connectionVisual`); a friend's count sits after the name; the server row scrolls at the dock's full height; folders sum mentions and keep their count when selected; Add friend opens Incoming while requests wait (`showFriendsManager(tab:)`, public `FriendsManagerTab`); hover is a surface step, never a bar; an empty DM has no time (`at: null`, `conversationTimeLabel` returns '' for the epoch).
  - **A, places and tools:** the five `ShellTab`s are one group in the dock (the Shop only behind `shopAvailableProvider`), folding into one Places menu under a 1000 px dock; tools are Downloads (a count badge), Help and Settings (Profile, as Classic). `openShellTabProvider` names the open tab. Saved messages is Home's first row on desktop. The recovery key left the dock; Needs Attention carries "Your recovery phrase isn't backed up yet" once the checklist is hidden or finished.
  - **B, the header is the title bar in Dock mode:** `DesktopWindowFrame` (`app.dart`) drops the 32 px bar while `_DockChromeClaim` holds `dockOwnsWindowChromeProvider` (not before an identity loads, so Welcome and the password prompt keep it, nor while locked, in annotation or fullscreen; a dialog over the dock gets a drag strip over the header from the `routesAboveHome` navigator observer) and floats `WindowControls` unscaled at the top-right at 44 x the effective zoom; they report their width and the header reserves it. The "Hollow" wordmark is gone, the restore glyph is two squares, close hovers red with a white cross, annotate has a name, focus and a label that never covers the zoom readout, and the controls run in one order on every platform. **macOS got it too:** `MacTrafficLights` sends the header height over `hollow/traffic_lights` and `MainFlutterWindow.swift` centres the native lights in it (Electron's trafficLightPosition method, re-applied on resize, key and fullscreen exit). Verified on the mini with real CGEvent clicks: header buttons take clicks, the middle drags the window, a double-click zooms and restores, the lights stay centred. Vitalik saw it: "Pretty cool!"
  - **C and D:** `DockIdentity` + the call at the left; the server holding the call wears `VoiceHereBadge`; ONE `NavSelectionMark` (shared with the phone's tab bar) from `dockLocationProvider`; Home is `HollowMark` (the logo with the padlock and keyhole, a painter, readable at 22 px) on an `elevated` tile; servers anchor after Home, Add is a ghost tile after the list.
  - **Polish:** `friendsBarProvider` (name order, case-insensitive everywhere via `compareFriendNames`; favourites, a divider, unread non-favourites, "+N more"; stale favourites fall back to all), the chip's status-line tooltip and fill-following cut-out, `EdgeScrollRow` (`fadeColor`, `shrinkWrap`, Lucide chevrons), `HollowCountBadge` no longer stretches in a Row, the status banner (roles, tiers, `HollowIconButton` dismiss and chevron, info is grey), `UserBar` (roles, `HollowIconButton`s, tooltip, no accent on the budget bar), `ServerFolderIcon`, `ServerIconImage` and `NewServerEntry` tokens.
  - **Guards:** baselines font-size 493 to 478, material-colors 171 to 166, color-literal 126 to 125, radius 92 to 88, edge-insets 189 to 182, sized-box-gap 146 to 135, gradient 16 to 15, raw-material 26 to 24. Tests: `dock_bar_test.dart` (one mark per location, hover adds none, folder @, full-height viewport, voice controls, places toggle, Shop absent, the fold), `friends_bar_test.dart` (replaces the toggle test), `home_inbox_test.dart`, `window_chrome_test.dart`, `time_labels_test.dart`. Probe: a `zoom` op; `fleet/dock_header.json` shoots every place, 150% zoom and the close corner; every scenario that found a friend chip by `tooltip:<name>` now uses `semantics:<name>` (the tooltip is the status line).
  - **Left for later:** the Linux laptop check of the drag and double-click on X11 and Wayland (the laptop was off; Linux shares the Windows code path); the Friends Manager dialog (local tab chip, a red Remove at rest, Title Case "Send Request" / "Add Friend": the dialogs pass, and ~37 scenarios tap those strings); Classic's `ServerStrip` (its own pass); drag-to-reorder still needs a 300 ms hold; `AwaitingSetupBadge`'s 10 px glyph.

- **Session 16 (2026-09-24): Settings and the Friends Manager, desktop and phone.** Mockup https://claude.ai/artifact/VsHS3eCdhVjjB1vtL2b9Jc (Vitalik: "freaking AMAZING", "definitely option C").
  - **Settings is a centre PLACE** (`ShellTab.settings`, `settings/settings_place.dart`), not a dialog: dock, header and a call's controls stay in reach. `openSettings()` / `toggleSettings()` (`core/providers/settings_place_provider.dart`) serve the gear, Classic's user bar, Ctrl+, the tray, Home and the profile card; unlike the other places it keeps the selection underneath, so closing (gear, Escape, the X) returns to the conversation. The gear carries the selection mark; Settings never folds into the Places menu. Design language 5.3.1 and 4.4 updated.
  - **Layout C:** a 240 rail and a 720 page column; on a window wider than the pair the PAIR centres, the rail's `surface` bleeding to the left edge (5.2 gained the one exception). Rail items 32 tall so all eleven fit a 768 px laptop screen (the first render clipped About).
  - **13 categories to 11 in groups** (`settings_catalog.dart`): Account (Profile, Security, Devices), App (Appearance, Accessibility, Notifications, Audio & Video, Shortcuts), Connection and data (Network, Files & Storage), About. Backup into Security, Updates into About, profile folders into Files & Storage, Appear invisible into Profile. Rail icons kept in grey; rows lost theirs. Search finds single settings (`kSettingsSearchIndex`).
  - **`settings_kit.dart`** is the one row: `SettingsPage`, `SettingsSection`, `SettingsRow` (+ `titleTrailing`, `monoTitle`), `SettingsSwitchRow`, `SettingsChoiceRow`, `SettingsSliderRow`, `SettingsExpandRow`, `SettingsAdvanced`, `SettingsNote`, `SettingsDensity(touch:)`. No cards, one line of explanation per row, rare knobs in one Advanced fold, danger zone last.
  - **Profile draft** (`profileDraftProvider`) owns every unsaved edit, so leaving keeps it; one "You have unsaved profile changes" bar (desktop) or Save in the phone's bar. The old dialog silently DISCARDED unsaved edits on close.
  - **The phone hosts the same pages** (`mobile_settings_tab.dart` 4,097 to 355 lines); phone-only flows (PIN/biometric app lock, backup save sheet, iOS log export, your ID, the crop route) live in the shared pages behind platform gates.
  - **Friends Manager** (`dialogs/friends_manager_dialog.dart`): three tabs (Friends with favourites on top, Requests, Add friend), row actions on hover, Remove in More with a confirm (it removed instantly before), favourites reorder fixed (filtered indices moved the wrong entry). Phone Friends tab to parity. Sentence case; ~20 probe scenarios updated.
  - **Verified:** `fleet/settings_before.json` / `settings_after.json` (desktop, two throwaway peers: every page, search, the unsaved bar across pages, 67% and 150% zoom, light) and `fleet/settings_mobile_before.json` / `settings_mobile_after.json` (the mini). Pairs `build/cmp_settings_*.png`, `build/cmp_settings_mobile_*.png`. 1,178 tests green; guard baselines font-size 478 to 331, material-colors 166 to 158, color-literal 125 to 122, radius 88 to 73, edge-insets 182 to 151, sized-box-gap 135 to 103, gradient 15 to 13, raw-material 24 to 23.
  - **Vitalik's review round (same session):** spacing alone read as clutter, so `SettingsPage` draws a hairline between its big sections; explanations had been cut past meaning ("Voice / Music / Hi-Fi" with no kbps), so every choice row describes its selected option and security/privacy rows keep every fact; Support marks became its own section; Your art thumbs share the Profile rows' 48 slot (banners 2.5:1, frames inside the slot); a pack can be removed (More, confirm; Rust `remove_owned_art`, local only); the pair centres on the rail's box edge, where the title now sits; the relay stat value hugs its edge again (a Larger Text fix had pushed it mid-row). Rule: `feedback_settings_sections_dividers_explanations`.
  - **Left for later:** Server settings onto the same shell; a background image shows "Custom image" after a restart (the provider keeps no name); devices lack last-seen and kind in `MyDevice`; the storage legend needs categorical colour tokens; the phone friend sheet has no Voice call; phone buttons in request rows are 38 px (a touch height on `HollowButton`); `UpdateState` has no `lastChecked`. One Flutter test failed in CI after commit b4614287. All but Server settings closed in session 17 (below); device kind and last-seen dropped.

- **Session 17 (2026-09-24): session 16's leftovers, and the Server settings mockup.**
  - **CI failure:** `settings_devices_about_test` "a ready update offers install and restart" failed only on Linux: `isFlatpakInstall` calls Rust through `linuxInstallKind()` behind `Platform.isLinux`, and widget tests never initialise the bridge. The test now sets `linuxInstallKindOverride` (`feedback_widget_test_platform_gated_ffi`).
  - **Leftovers closed:** the background image keeps its name across a restart (`BackgroundState.imageName`, setting `bg_image_name`); `HollowTheme.categorical` (three hues plus a neutral, 3:1 on every surface in both themes, pair-checked) colours the storage bar and legend; the phone friend sheet offers Voice call to an online friend (`startMobileDmCall()` shared with the DM header); `HollowButton(touch: true)` is 44 tall on phones (Friends tab and Friends Manager request rows); `UpdateState.lastChecked` + `relativeTimeLabel()` give About "Last checked 3 hours ago". Device kind and last-seen were DROPPED (kind needs a wire field; Vitalik: not now). 1,191 tests green.
  - **Server settings:** audited on throwaway peers (`fleet/server_settings_before.json`, shots `ssb-*`): squeezed into the chat pane between channel list and member panel, a tab strip that scrolls at 1280, "Server Settings" twice, five Saves on Overview, tinted role cards, raw ids on member rows, a red Danger tab and card. Mockup https://claude.ai/artifact/GZmWGic3kxaDw6ug6pRuCG **APPROVED**: the Settings shell with the rail in the channel list's place, Server group (Overview, Access, Channels, Roles as one table, Labels, Emotes and stickers, Members) and You group (Profile, Notifications), one unsaved bar, Members with Moderation first then one flat searchable list with role chips, phone list rows with icons. Vitalik asked whether rail + page is too Discord-like; kept, since it is the standard settings layout (macOS, Windows, Slack, Teams, Telegram) and the differences live in the place model, the table, inline channel rows and the copy. The full build brief is `tmp3.txt` at the repo root. **Next: build it.**

- **Session 18 (2026-09-24): Server settings built, desktop and phone.** Everything in session 17's brief (`tmp3.txt`), to the approved mockup.
  - **One shell for both places:** `settings/settings_place_frame.dart` (`SettingsPlaceFrame`: rail + bleed + page column + close + one floating bar; `SettingsRail`, `SettingsRailItem`, `SettingsRailGroupLabel`, `SettingsUnsavedBar`). Settings and `server_settings/server_settings_place.dart` both sit on it.
  - **The place model:** `serverSettingsOpenProvider` stays the one switch; `openServerSettings(read, serverId, {page})` sets the target server explicitly (a split's right pane reads its OWN selected server) and `serverSettingsTargetProvider` resolves it. The shell mounts the place in Classic (strip stays), Dock and split (settings take the centre; the 800x600 dialog is gone). A server that is not the selected one (right pane, phone long-press) renders inside `ForeignServerSettingsScope`, which gives the pages that server's own channel list and layout.
  - **State:** `server_settings_provider.dart`: `serverSettingsDraftProvider(serverId)` (name, description, member limit, nickname through ONE unsaved bar; the member-count check stays) and `channelLayoutDraftProvider(serverId)` (order, categories, dividers staged for "Save layout", written through `ChannelLayoutNotifier.mutate`; channels added elsewhere just appear, no auto-save needed). Switches, chips and menus write at once: private, adult, catch-up and all six Twitch keys are per-key writes now (turning Twitch on with no channel asks for it first).
  - **Pages** (`server_settings/pages/`): Overview (icon/banner rows, fields, invite preview, Advanced: id + template, Danger zone), Access, Channels (summary by exception via `channelSummary()`, inline panel, category/emote/sticker removal now confirm), Roles (one table, `rolePermissionsProvider` reads Rust defaults), Labels, Emotes and stickers, Members (moderation first on the kickMembers bit, one flat list with role chips, capped at 100 rows with a search note), Profile (nickname + live line, labels to wear, Leave), Notifications. Private server copy corrected to what Rust does: nobody new can join, even with a link.
  - **Phone:** `MobileServerSettingsRoute` is the list (server header, grey icons, values) pushing the same pages at touch density; the five phone-only sub-routes are deleted. `MobileSettingsSubPage` / `MobileSettingsNavRow` are shared with Settings.
  - **Deleted:** `server_settings_panel.dart`, the eight `*_tab.dart` files, `mobile_{members,roles,labels,emotes,twitch_settings}_route.dart`. `ChannelOverrideDropdown` moved to its own file.
  - **Twitch channel by name (Vitalik, same session):** the Access dialog looks the numeric id up as you type the channel name (`twitch_lookup_channel`, Helix `/users?login=`). Helix has wanted a Bearer token on every call since 2020, and the app has no client secret, so the lookup rides the CONNECTED account's user token; without one it says to connect Twitch and the id field stays editable. Logins are checked against Twitch's 1 to 25 `[A-Za-z0-9_]` shape before they reach a URL. The Twitch row carries the purple brand icon.
  - **Probes:** a `reveal` op (`ensureVisible`) in the probe runner; every fleet cleanup that deleted a server through the Danger tab now reveals Overview's Delete server and confirms at dialog index 0 (8 journeys, 9 scenarios). New `fleet/server_settings_after.json`, `server_settings_classic.json`, `server_settings_after_mobile.json`.
  - **Verified:** 12 new widget tests (`test/widget/server_settings_test.dart`), full suite green, guard baselines font-size 331 to 307, material-colors 158 to 155, color-literal 122 to 112, radius 73 to 71, edge-insets 151 to 143, sized-box-gap 103 to 93, raw-material 23 to 21. Driven on throwaway fleet peers (desktop Dock and Classic, light, 150%, a wide window, the member view) and the mini's simulator; pairs in the session scratchpad `cmp/`.

- **Session 19 (2026-09-25): the Shop, and session 18's three follow-ups.**
  - **Mockup** https://claude.ai/artifact/BCFHRKNJh5kRnMSi7Trd8B **APPROVED** (Vitalik: "I love it"): shelves over the mixed wall, no kind badge on a card (the shelf names it), banners two across.
  - **Built** (`shop/shop_dashboard.dart`, new `shop/shop_art.dart`, `shop/shop_item_dialog.dart`): one shelf per kind (avatars and frames square, banners and bundles at 2.5:1), All shows one row per shelf with See all; the art is the card (full bleed, title and price, artist and Owned beneath); frames on a plain face or, with the shelf's switch, on your own avatar; bundles drawn as one look; header down to one filled Redeem a code plus a More menu (Import a pack, Open the shop in your browser, Refresh, Your art in Settings); the tinted strip is the intro line ("made by real people"); skeleton shelves while loading, an empty state on error. The item view shows the piece on YOUR profile card; a dialog on desktop, a sheet on phones.
  - **Buy goes straight to Ko-fi:** the catalog's `buy_url` is parsed in Rust (`ShopListing.buy_url`, rebuilt from the item code, anything but `https://ko-fi.com/s/<code>` refused), the button reads "Buy on Ko-fi". **Open in browser worked only on the second tap:** it read an unwatched `FutureProvider` with `valueOrNull`, so the first read only started the lookup; it now awaits `.future`.
  - **Mobile gate unchanged and confirmed:** iOS never shows the Shop, Android only for a sideloaded APK (Play, Amazon and Samsung installers excluded); marks keep rendering everywhere.
  - **Follow-ups (background agent):** the phone invite preview capped at 232; Members is a lazy `SliverList` via `SettingsSliverPage`/`SettingsScrollView`, no cap; Classic keeps the `UserBar` at the foot of the settings rail (Settings and Server settings).
  - **Verified:** Shop tests 7 (was 5), `test/screenshots/shop_screenshot_test.dart` (desktop and phone shelves, dialog, sheet; `HOLLOW_SHOP_ART_DIR` for real pictures), Rust `buy_url_is_only_ever_a_kofi_item`; fleet `shop_redesign` on a throwaway peer against the live catalog; guard baselines font-size 307 to 306, material-colors 155 to 154.
...
**Sweeps 3 to 9 are done** (sessions 4 to 8). **Session 9 wrote the screen standards, the Home mockup (approved) and built Home on desktop; session 10 brought it to the phone's Chats tab (Vitalik: "looks AMAZING"); session 11 closed the three Home follow-ups; session 12 redesigned the chat screen on both platforms (above); session 13 did the animation pass; session 14 audited the dock and header and mocked them up; session 15 built them; session 16 rebuilt Settings as a place and the Friends Manager (below); session 17 closed its leftovers and got the Server settings mockup approved; session 18 built Server settings; session 19 built the Shop. Next: Archive, Share and Conferences as one session.**

**Next session (session 19): the Shop, plus the Server settings follow-ups in the background.** The Shop is structural: audit, a clickable mockup on a Design canvas (the per-kind card shapes of verdict 9: the art is the card, full bleed, title and price under it, one badge for kind and one for owned, the header row down to one primary), Vitalik's verdict, then the build, desktop and phone together. While that runs, ONE background agent takes the three small things session 18 left, each verified on renders:

1. **Phone invite preview is too big.** On the phone, Overview's "How an invite shows it" card runs the full width (`server_settings/pages/overview_page.dart`, `_InvitePreview` under `SettingsDensity.touch`). Cap it at the desktop width (232) or a phone-sized fraction, left-aligned, so it reads as a preview and not a banner.
2. **Classic hides the user bar while a place is open.** In the Classic layout the channel sidebar's user bar (you, status, downloads, the Settings gear) disappears while Server settings is open, because the place replaces the whole sidebar; Settings does the same. Decide with Vitalik whether the rail keeps a user bar at its foot in Classic (Dock is unaffected: the dock holds you).
3. **Members caps at 100 rows.** `members_page.dart` draws at most 100 rows and asks for a search, because the settings host scrolls the whole page in one `SingleChildScrollView`. Replace it with a real lazy list (a sliver-based host for that page, or a fixed-height `ListView.builder` region) so a thousand-member server scrolls without the note.

Then the remaining screens (Archive, Share, Conferences as one session; Voice channel pane and call surfaces as another), then the dialogs pass (start with an audit agent listing which still break the rules; Friends Manager and the sweep 8 frame are done).
...
3. ~~Dock and header~~ done in session 15 (structural after the session 14 audit, see above).
