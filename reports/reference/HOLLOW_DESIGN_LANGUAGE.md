...
**Tab rows are chips** (decided 2026-09-25, Vitalik), in a dialog body, a page or a place header: `HollowChipTabs<T>` (`tabs`, `selected`, `onSelected`; a quiet total in `hint`, something waiting on the person in `count`, a count badge; arrow keys, Home and End move the selection; `expand: true` for equal widths on a phone). **The one exception is a row that IS a surface's header and splits the whole surface into its sections** (the expression picker's Emoji, GIFs and Stickers; the Friends manager's Friends, Requests and Add friend; refined 2026-09-26, Vitalik: chips there "feel out of place" and "Add friend" read as a button): `HollowTabBar<T>`, the same `HollowChipTab`s as equal tabs across the top with a 2 px accent bar under the open one, sitting on the divider below it. Anything that filters or chooses inside content stays chips, so a chip row under a tab bar reads as a second level. No other underline tab, no local `_Tab`. A label is `LabelChip` when it toggles and `LabelBadge` when it is worn, both led by the label's colour (`LabelSwatch`); `HollowDurationPicker` is the one "for how long" choice, a chip row whose null is "Until I remove it", never a red "Permanent".
...
- **A main region plus at most one side panel.** Three regions of equal weight have no winner. The side panel holds secondary, glanceable things and never the screen's job. It is built like a server's member panel: `surface`, a hairline on its inner edge, full height, starting 280 to 300 wide with a seam on its inner edge the person can drag (the width is saved). A panel beside a call stage (`CallStageWithPanel`) yields to the stage on a small window rather than squeezing the tiles. A navigation sidebar is 240.
