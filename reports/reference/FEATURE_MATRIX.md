...
| 290 | Relay kill list (offline destroy signal) | `kill_list.h`, `destroy.rs`, `ws_client.rs` | Done | None, it is a courier | A destruction order parked for a device that is away survives a relay restart in the snapshot and is handed over on that device's next auth; the client verifies it against its own master, refuses anything else and acks so the entry dies. `scripts/fleet_relay_restart.ps1` G5 (parked, restarted, delivered, refused, acked, not re-sent) and G6 (a guest socket is refused) |
| 291 | Destroy my identity everywhere | `duress_section.dart`, `wipe.rs`, `destroy.rs` | Done | Settings > Security > Danger zone, type DESTROY | One signed order reaches every device of the identity: a sibling that is online wipes and restarts within seconds, a device that was away takes the same order from the relay's kill list at its next auth, and friends who were told get a standing "This identity was destroyed" banner with their verification of that contact cleared. Driven end to end on the real relay by `scripts/fleet_destroy.ps1` (G1 two linked devices, G2 a verified friend, G3 a third device offline, G4 both live devices wiped and exited, G5 the friend's banner, G6 the offline device on its next auth, G7 both back at Welcome with empty data roots) |
