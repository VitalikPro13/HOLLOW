...
- **Profiles/avatars:** profiles load light (`getAllProfilesLight()`, no blobs); `HollowAvatar` auto-fetches from `avatarProvider` — never pass `imageBytes:`; banners via `bannerProvider`; `ProfileUpdated` invalidates BOTH caches. `feedback_lazy_avatar_pattern`.
...
- **Perf sentinels:** grep `[SENTINEL]`; `timedChannelCall` on direct-channel calls. `project_perf_sentinels`.
...
- **CRITICAL: relay per-IP accounting goes through `ip_limit_key()`** (v4 addr / v6 **/64**; unmap v4-MAPPED `::ffff:…` FIRST, else naive truncation collapses ALL v4 into ONE bucket). 10 GB/day counts binary BOTH directions; exhaustion = 1008 close. `project_relay_bandwidth_enforcement`, `project_relay_ipv6`.
...
- **CRITICAL: MLS epoch staleness:** sync requests use plaintext `HavenMessage`; CRDT broadcast falls back to plaintext on MLS failure; stale-PRESENT groups heal via commit catch-up (hints NEVER drop a group; frames only at own+1; commits ONLY via `broadcast_mls_commit`). `project_mls_epoch_catchup`; all 8 MLS rules in `feedback_mls_patterns`.
...
- **CRITICAL: Windows mid-call media:** `addTrack`/`removeTrack` + renegotiate, NEVER `replaceTrack`. Live `setParameters` WORKS: never write rid/ssrc back on the live path, skip ""/0 sentinel optionals both directions; init `sendEncodings` = guaranteed path. `project_webrtc_engine_screenshare_research`.
...
- **CRITICAL: media forwarder lane (`fwd_*`, own `fwd:{id}` room):** CLIENT legs = **ZERO ice servers**, EXEMPT from Always-relay (media never touches a member's machine in ANY role). MEDIA LEGS = truth; assign trust = ORIGINATOR; feeder = SUPPLY NEVER AUTHORITY. Simulcast/caps/room-hygiene + all field iron rules: `project_media_forwarding_epic`.
...
- **CRITICAL: context menus = ONE surface, `showHollowMenu`** (`components/hollow_menu.dart`, #61): dialog route (raw OverlayEntry dies under `SelectionArea`); anchor via `overlayPositionOf`; rows = BUILDER using `menuRef`, NEVER shadowing the caller's `ref` (CI-guarded); dismiss by route IDENTITY, action next frame. `project_issue61_context_menus`.
- **CRITICAL: channel layout writes go through `ChannelLayoutNotifier.mutate` ONLY** (never direct `updateChannelLayout`): normalise + optimistic publish + guard against BOTH `loadForServer` AND `setLayout(json, serverId:)`. `feedback_one_mutation_path_per_state`.
...
- **CRITICAL: per-channel MLS subgroups:** restricted channels encrypt under their OWN group `"{server}#{channel}"` (`reconcile_subgroups_for_server`); voice SFrame from its `export_secret`; VC participants (SELF too) = ROUTABLE DEVICE ids, never MLS leaf/master; Dart branches on `is_self`, NEVER id compares. `project_per_channel_mls_subgroups`.
...
- **CRITICAL: `RoomMembers` = authoritative presence:** diff sets → `PeerDisconnected` for vanished; `PeerLeft` refreshes suspect rooms; membership caches tolerate missed `PeerLeft`; OUR OWN leave purges the room (`WsEvent::LeftRoom`) — a stale self-left room silently blackholes targeted sends. `feedback_ws_presence_stale_rooms`.
...
- **Widget tests:** `pumpHollowMobile()` (`test/helpers/test_app.dart`) mocks FFI (~1s, no device). **UI probe** (drives the REAL app, screenshots to `build/ui_probe/`): `pwsh scripts\ui_probe.ps1 -Scenario <x>`, `project_ui_probe_integration_harness`. **Feature matrix:** `reports/FEATURE_MATRIX.md`.
...
- **Flatpak:** `flatpak/` + `build-flatpak.sh`; requires `--socket=x11` (NOT `fallback-x11`) and `--socket=session-bus`+`--own-name` (else GApplication register fails and it never launches, #59); must bundle libsecret (`feedback_flatpak_libsecret_and_vm_no_gui`).
