...
- **AI noise suppression LIVE:** RNNoise default (DFN3 = desktop Engine selector), `hollow_dfn`+`dfn_ffi.rs` ABI v3, WebRTC NS auto-disabled + fallback. Capture buffers = ALWAYS fullband mono (`num_bands`=APM metadata — ports pass bands=1); RNNoise VAD gates upward boost (legacy gates fallback); `frames>0` or the test didn't happen. See `project_dfn3_noise_suppression`.
