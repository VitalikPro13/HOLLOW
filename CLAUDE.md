...
- **CRITICAL: user avatar/banner ANIMATION rides the rail (`AssetKind::Profile`), only the STILL rides the push:** hash absent = PRESERVE, `""` = clear; NOT signed; ceilings never upscale; render `imageBytes` > rail > still, so a held animation leaves `avatarProvider` EMPTY. `project_profile_media_asset_rail`
- **CRITICAL: animated encoding = `node/webp_anim.rs`.** EVERY processor decides animation from BYTES via `is_animated_image` (GIF+APNG+WebP) — a `GIF8`/extension branch silently FLATTENS APNG; `is_animated_webp` = passthrough only. `project_animated_avatar_encoding`.
...
- **Perf sentinels:** grep `[SENTINEL]`; `timedChannelCall`. `project_perf_sentinels`.
...
- **CRITICAL: multi-device device-vs-master routing.** One MASTER = many DEVICE peer_ids: per-person UI/lookups collapse device→master (`resolver::resolve`/`identityOf`); SENDS target a DEVICE, never the bare master — fan via `devices_for(master)`. Fresh installs ALWAYS have device≠master. `feedback_multidevice_targeting_sweep`.
...
- **CRITICAL: desktop libwebrtc (dll+so) is OUR patched build**, vendored at `third_party/libwebrtc/` (recipe BUILDING.md). Shares ride `ScreenContentProfile`, NEVER hint 'motion'. Encoding caps ride `addTransceiver` init `sendEncodings` (pre-negotiation setParameters is DROPPED at O/A). `project_webrtc_engine_screenshare_research`.
