...
- **Storage layout:** data root = `dirs::data_dir()/hollow` (`keys.rs::data_dir`; override via `set_data_dir()`/`HOLLOW_DATA_DIR`; NOT `~/.hollow`); mobile = sandboxed app documents (iOS in the App Group; Dart `hollowDataDir` passed via `set_data_dir()` before `start_node()`). Subfolders: `files/`, `vault/`, `vault_cache/` + `identity.key`/`identity.device` + `messages.db`.
