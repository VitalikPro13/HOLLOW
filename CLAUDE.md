...
- **CRITICAL — mic gain/loudness:** WebRTC APM AGC DISABLED — NEVER re-enable. Voice Enhancement owns loudness (`setCaptureGain`/`setVoiceEnhance`; `setVolume()`=NO-OP on capture): chain+servo → −16 dBFS + gate/upward + de-esser, 3 ports IN SYNC, tune ONLY via the g++ harness. NEVER a per-sample adaptive leveler (crackles), NEVER bypass iOS VPIO. See `project_voice_agc_loudness_rvox`, `project_voice_upward_compression`.
- **AI noise suppression (scaffold shipped, NOT LIVE):** head-of-chain post-AEC via `rust/hollow_dfn`+`dfn_ffi.rs`; WebRTC NS auto-disabled + fallback. Windows hook=16 kHz, Android=split-band — adapter pending. Voice field tests INVALID until log shows `frames>0`. See `project_dfn3_noise_suppression`.
