...
- **CRITICAL — multi-device data channels (`webrtc_service.dart`, hollow-data):** glare tiebreaker compares MASTER identities; answer/ICE match by `conn_id`, never peer_id; sends/sockets/map keys stay DEVICE-keyed. `sendScreenAudio` DROPS packets when `bufferedAmount` >256KB — desktop polls every 12 sends (`feedback_webrtc_datachannel_multidevice`, `feedback_screen_audio_datachannel_backpressure`). VC mesh glare is the OPPOSITE: ROUTABLE DEVICE ids, never master (`feedback_vc_join_double_announce_race`).
...
- **Roles & permissions:** 4 power roles + cosmetic labels; `role_permissions` overrides `default_permissions()`; tier-gated editing — `project_roles_design`. **CRITICAL — `AdminLwwReg::merge` = pure HLC LWW** (authority ONLY in op_allowed/author gates; priority = inert wire metadata); Dart loads role defaults via `default_role_permissions()` FFI, never a mirror — `project_crdt_hlc_lww_merge`. **Moderation trio:** mute (master-keyed) / slow mode (Mod+ exempt) / media-only — enforce at send + LIVE ingest, never sync backfill; mute also gates EDIT + ADD-REACTION on both crypto paths; slow-mode cooldown DERIVED from the message list — `project_moderation_trio`.
