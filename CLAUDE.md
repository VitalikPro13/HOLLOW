...
- **`.hollowpack` + Shop + support creds:** identity = sha256 of PROCESSED bytes; CLI `rust/hollow_art`; `import_hollowpack` verifies, NEVER re-encodes. Shop UI ONLY behind `shopAvailableProvider` (store builds render marks only). Credentials ride the profile as `support_creds`, verified OFFLINE against the PINNED root; `sanitize_incoming_support_creds` = the ONE validator (chain + THIS master); chain checked BEFORE spending; burn ONLY on Creem's "limit" text; Owned = a CREDENTIAL; one per listing. `support_creds_own` is PER-DEVICE: publish = UNION(table, shelf, own row) minus tombstones, hidden filters t in {1,2} (else siblings WIPE marks). Twitch = the SAME credential: chip ONLY from a verified t=3 (`twitchLoginProvider`), join gate = t=4 verified OFFLINE vs the joiner's MASTER; `support_creds_sig` + per-master pin (`gated_support_creds`), REJECT = preserve. `project_twitch_signed_credential`. wiki `hollowpack`, `support_credentials`.
