...
- **Roles & permissions:** 4 power roles (Owner/Admin/Moderator/Member) + cosmetic labels; `role_permissions` overrides `default_permissions()`; tier-gated editing. See `project_roles_design`. **Moderation trio:** mute (master-keyed lazy expiry) / slow mode (Mod+ EXEMPT) / media-only — enforce at send + LIVE ingest, NEVER sync backfill; mute also gates EDIT + ADD-REACTION on send+ingest, MLS AND Olm paths (delete/remove-reaction never; slow/media-only = new msgs only); slow-mode UI cooldown DERIVED from the message list, never send-time state. See `project_moderation_trio`.
