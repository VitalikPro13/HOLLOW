...
- **`.hollowpack` + Shop + support creds:** identity = sha256 of PROCESSED bytes; CLI `rust/hollow_art`; `import_hollowpack` verifies, NEVER re-encodes; owned hashes PINNED. Shop UI ONLY behind `shopAvailableProvider` (store builds: marks RENDER only). Credentials ride the profile as `support_creds`, verified OFFLINE against the PINNED root; `sanitize_incoming_support_creds` = the ONE validator (chain + THIS master); chain checked BEFORE spending; burn ONLY on Creem's "limit" text; Owned = a CREDENTIAL, not the library; one per listing. `support_creds_own` is PER-DEVICE: publish = UNION(table, shelf, own row) minus tombstones, hidden = the explicit clear (else siblings WIPE marks). wiki `hollowpack`, `support_credentials`.
...
- **Icons:** `lucide_icons_flutter` (camelCase), `brand_icons.dart`, `atlas_icons` (snake_case).
