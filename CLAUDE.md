...
- **CRITICAL — never construct `TextSelectionControls` in `build()`** (identity churn recreates selection overlays → app-wide crash inside raw OverlayEntries); raw-OverlayEntry teardown needs a `removed` guard (double-tap fires before the removal frame). See `feedback_textfield_overlay_selectioncontrols`.
