...
- **Theme:** Dark primary + light secondary. `HollowTheme.dark()`/`.light()` + `darkWithHue()`/`lightWithHue()` for custom accent. `themeModeProvider` persisted to SQLCipher via `ThemeModeNotifier` (loaded at bootstrap).
...
- **Mobile voice channels:** `mobile_voice_channel_route.dart` — full-screen voice view pushed via `PageRouteBuilder` slide-from-bottom (same as DM `MobileCallScreen`). Tap voice channel in accordion → `joinChannel()` + push text chat + push voice route on top. Back arrow reveals text chat underneath. `MobileVoiceChannelPill` (floating pill with mute/deafen/leave) lives in both `MobileShell` Stack and `MobileChatRoute` Stack — voice route covers it by being pushed on top. `VoiceChannelState.currentChannelName` resolved at join time (not from `channelListProvider` which is server-scoped). Shared widgets (`MobileClusteredAvatars`, `MobileSpeakingAvatar`, `MobileControlButton`) in `mobile_voice_avatars.dart` — used by both DM calls and voice channels.
- **Mobile image crop:** `mobile_image_crop_route.dart` — full-screen route with fixed crop frame + pinch-to-zoom/drag image underneath. Clamped so the crop frame never shows empty space. Used for avatar (1:1), banner (3:1), and background (9:16 on mobile, 16:9 on desktop). `showMobileImageCrop()` entry point.
- **Mobile background image:** `MobileShell` and `MobileChatRoute` both render the background image from `backgroundProvider` via a Stack: image → darken overlay (`panelOpacity` 0.0–0.92) → transparent scaffold. Pushed routes must include their own background layer since they cover the shell entirely.
- **Mobile storage dashboard:** `mobile_storage_route.dart` — full-screen drill-down from server settings. Shows server storage bar, your pledge (editable), retention policy, vault health. Same data as desktop `storage_dashboard_dialog.dart`.
- **CRITICAL — mobile floating pills must NOT go in `app.dart` builder.** `app.dart` builder is above the navigator — no pushed route can cover widgets placed there. Pills must be in `MobileShell` Stack (tabs) + `MobileChatRoute` Stack (chats). Full-screen voice/call routes push on top via `PageRouteBuilder` and cover pills naturally. Only `IncomingCallOverlay` belongs in `app.dart` builder (must be above everything).
