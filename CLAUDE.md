...
- **CRITICAL: MLS epoch staleness:** sync requests use plaintext `HavenMessage`; CRDT broadcast falls back to plaintext on MLS failure; stale-PRESENT groups heal via commit catch-up (hints NEVER drop a group; frames only at own+1; commits ONLY via `broadcast_mls_commit`). `project_mls_epoch_catchup`, `feedback_mls_patterns` (all 8 rules).
...
- **CRITICAL: context menus = ONE surface, `showHollowMenu`, opened via `ContextMenuTarget`** (`hollow_menu.dart`, both CI-guarded): dialog route (OverlayEntry dies under `SelectionArea`); anchor via `overlayPositionOf`; builder ref = `menuRef`, never the caller's; dismiss by route IDENTITY, act next frame. `project_issue61_context_menus`.
