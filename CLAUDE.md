...
├── packages/flutter_webrtc/ # FORK 1.5.2, stock libwebrtc m144; screen_audio_capturer.exe =
│                         # SEPARATE target. See project_flutter_webrtc_152_upgrade
...
- **Performance sentinels (self-diagnosis):** quiet-by-default anomaly loggers, grep `[SENTINEL]` in hollow_debug.log — Dart `perf_sentinel.dart` (frames + channel calls; direct-channel sites use `timedChannelCall`), Rust `sentinel.rs` (swarm/heartbeat/backlog), native chain → status map. See `project_perf_sentinels`.
...
- **AI noise suppression LIVE:** RNNoise default (DFN3 = desktop Engine chip), `hollow_dfn`+`dfn_ffi.rs` ABI v3, WebRTC NS auto-disabled + fallback; RNNoise VAD gates upward boost; `frames>0` or the test didn't happen. Capture buffers = ALWAYS fullband mono (`num_bands`=APM metadata) — ports AND enhance chain treat `rate/100`-sized buffers as bands=1. See `project_dfn3_noise_suppression`, `project_voice_enhance_chain`.
...
- **CRITICAL — audio-track `setEnabled`/`setVolume` = blocking signaling-thread hops** (LiveKit releases the mic while tracks disabled → 2s freeze): run via sentinel-timed `runAudioTrackOp` (Android) / `HollowRunAudioTrackOp` (darwin), never raw execute/dispatch_async or the platform thread. FRB TRACE loggers capped at Warn in `init_app` — NEVER remove. See `feedback_android_audio_track_proxy_ui_freeze`, `feedback_frb_trace_logger_cap`.
