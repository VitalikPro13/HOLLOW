...
- **CRITICAL — Profile Showcase Board:** `showcase_board` wire field is `Option<String>` (absent = old client → PRESERVE; `""` = clear); asset bundle rides the avatar blob playbook (hash on light announce, bytes on full pull, content-addressed — game-card details live in the bundle, board text stays tiny). NO relational blocks (VETOED = Discovery). IGDB at AUTHORING only via `igdb/search.php` (send `v=SEARCH_VER`); **deprecated IGDB enum fields return NOTHING silently**. Showcase images = LOSSY WebP, NEVER the image crate's lossless. `showcase_fetch_cover/key_art` refuse non-Hollow-CDN URLs. See `project_showcase_board_impl`, `feedback_igdb_deprecated_enums_and_cdn_cache`.
