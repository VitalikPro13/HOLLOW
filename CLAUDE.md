...
- **CRITICAL — custom emotes:** content-addressed WebP blobs, wire token `[e:name:hash]` (grammar dual-defined: `node/emotes.rs` + `emote_image.dart`); bytes NEVER ride CRDT/envelopes/relay rings — pull via `EmoteRequest`→`EmoteAssets`, verified at receive. FFZ = AUTHORING-ONLY via the website `ffz/` proxy (default `curated=1`); receivers never fetch. Composer: 1 PUA char ↔ 1 WidgetSpan; send sites read `expandedText()`, NEVER `.text`. Gated by `MANAGE_EMOTES` (1<<7). See `project_custom_emotes_ffz`, wiki `emotes`.
