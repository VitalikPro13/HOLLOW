...
- **Node modules:** `swarm.rs` = event-loop dispatcher; domain logic lives in focused modules as `pub(crate) async fn handle_*()` called from swarm match arms; types/enums in `types.rs`. No SwarmContext struct — pass state vars individually (see `feedback_swarmcontext_borrow`). New envelope variants go in the right module's `handle_envelope_*()` (`feedback_envelope_dispatch_pattern`).
...
- **Accessibility (epic CLOSED).** Iron rules: (1) icon-only controls need a PURPOSE label + (2) focus rides `HollowFocusRing` — both CI-guarded. (3) Reduce motion only via `ReduceMotionController`; mobile routes via `hollowMobileRoute()`. (4) Larger text: only the 3 safe bar-fix patterns. (5) StatusDot = presence by SHAPE (`filled:`). (6) Never `dart format` mid-edit. See `project_accessibility_*`.
...
- **Deep links:** `hollow://` on all 6 platforms via `DeepLinkService` (init pre-runApp, buffers until shell ready). Invites COPY `webServerInviteLink()` (`…/join#server=ID` — FRAGMENT, id never hits server logs); ALL link input bars parse via `inviteIdFromInput`/`classifyHollowLink` (both forms); Windows forward = NO-ARG `SendAppLinkToInstance()` in main.cpp. See `project_deep_linking`.
...
- **CRITICAL — mic gain/loudness:** WebRTC APM AGC DISABLED (`autoGainControl:false`, AEC+NS kept) — NEVER re-enable. Loudness owned by Voice Enhancement (static chain + servo → −16 dBFS, 3 native ports IN SYNC) via `Helper.setCaptureGain()`/`setVoiceEnhance()` (`setVolume()`=NO-OP on capture). NEVER a per-sample adaptive leveler (crackles), NEVER bypass iOS VPIO. See `project_voice_agc_loudness_rvox`.
...
- **Roles & permissions:** 4 power roles (Owner/Admin/Moderator/Member) + cosmetic labels; `role_permissions` overrides `default_permissions()`; tier-gated editing. See `project_roles_design`. **CRITICAL — `AdminLwwReg::merge` = pure HLC LWW** (latest authorized write wins; authority ONLY in op_allowed/author gates, role priority = inert wire metadata); Dart loads role defaults via `default_role_permissions()` FFI, never a mirror. See `project_crdt_hlc_lww_merge`. **Moderation trio:** mute (master-keyed) / slow mode (Mod+ EXEMPT) / media-only — enforce at send + LIVE ingest, NEVER sync backfill; mute also gates EDIT + ADD-REACTION on both crypto paths; slow-mode UI cooldown DERIVED from the message list. Full rules: `project_moderation_trio`.
- **CRITICAL — per-channel MLS subgroups (Option B):** restricted channels encrypt under their OWN MLS group keyed `"{server}#{channel}"` — visibility is cryptographically enforced. Membership via coordinator-gated `reconcile_subgroups_for_server`; voice derives SFrame from the subgroup `export_secret`; auto-leave on lost visibility runs on BOTH CrdtOp apply paths; VC participants key on the ROUTABLE WS sender, NEVER the MLS leaf — display sites collapse `identityOf` for avatar/nickname. See `project_per_channel_mls_subgroups`, `vc_participant_display_master_collapse`.
...
- **CRITICAL — chat lists are `reverse: true`** via shared `reversedChatList()` in `chat_pane_shared.dart` (desktop panes + mobile; mobile passes `selectionArea:false` — SelectionArea fights touch long-press): newest = index 0 bottom-pinned, instant post-frame `jumpTo(0,0)` ONLY, scrolled-up reading FREEZES the display, `findChildIndexCallback` mandatory (guard test `chat_list_element_reuse_test.dart`); popped MobileChatRoute callbacks bail on `_routeDeactivated`. Extend the shared module, never copy between surfaces. See `feedback_reverse_chat_lists`.
