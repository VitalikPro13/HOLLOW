...
- **Node modules:** `swarm.rs` = event-loop dispatcher; domain logic in focused modules as `handle_*()` called from its match arms; types/enums in `types.rs`. No SwarmContext — pass state vars individually (`feedback_swarmcontext_borrow`). New envelope variants go in the owning module's `handle_envelope_*()` (`feedback_envelope_dispatch_pattern`).
...
- **CRITICAL — Dart traps that fail SILENTLY:** `CrossAxisAlignment.stretch` Row needs bounded height (wrap in `IntrinsicHeight`); `ref.listen` ONLY in build(); never `Flexible` beside `Expanded`; srcIn tints need TRANSPARENT ink; `whenComplete(() => map.remove(k))` self-deadlocks. `feedback_dart_patterns`.
...
- **Theme:** `HollowTheme.dark()/.light()` + hue variants; `themeModeProvider` persisted. Contrast: `accentText` for accent TEXT (raw `accent` = fills only), `textTertiary` for faded metadata — both 4.5:1 (`contrast.dart`, CI-guarded). Old window chrome hardcodes white — grep `Colors.white`.
...
- **CRITICAL — relay per-IP accounting goes through `ip_limit_key()`** (v4 addr / v6 **/64**; unmap v4-MAPPED `::ffff:…` FIRST — naive truncation collapses ALL v4 into ONE bucket). 10 GB/day counts binary BOTH directions; exhaustion = explicit 1008 close. `project_relay_bandwidth_enforcement`, `project_relay_ipv6`.
...
- **CRITICAL — "Always relay calls" (force-TURN) lives ONLY in `IceConfigNotifier`:** `ref.listen` the toggle, NEVER `ref.watch` (rebuild wipes cached TURN creds); fails closed; NEVER add TURN to `shareIceConfigProvider` (Share = own STUN-only PC) or to forwarder legs. `project_always_relay_calls`, `project_share_data_channel_reuse_turn`.
...
- **CRITICAL — multi-device device-vs-master routing.** One MASTER = many DEVICE peer_ids: (1) per-person UI/lookups collapse device→master (`resolver::resolve`/`identityOf`); (2) SENDS target a DEVICE, never the bare master — fan via `devices_for(master)`. Fresh installs ALWAYS have device≠master. `feedback_multidevice_targeting_sweep`.
- **CRITICAL — multi-device audit fixes:** sends route via `send_raw_to_identity` / `preferred_online_device` / `friend_device_targets` (raw master send drops). LEAVE tears down on all 3 apply paths; remote-op ingest gates via `ServerState::op_allowed`. Resolver tests hold `resolver::test_lock()`. `project_multidevice_audit_2026_07`.
- **CRITICAL — targeted sends route to their DETERMINISTIC room, never `ws_room_for_peer`** (first-match picks a LEFT/not-yet-joined room → silent one-way loss): DMs → `dm_room_code`, forwarder signals → `fwd:{id}`. Sibling self-echo keeps the flexible lookup. Friend requests queued on a COLD resolver drain on device-list ingest, not just presence. `feedback_dm_friend_establishment_bugs_2026_07`.
...
- **CRITICAL — screen shares are OPT-IN (#38):** a share streams ONLY after the peer's `screen_watch{want:true}` (VC `vc_screen_watch`; DM `call_screen_watch`); receivers DROP unsolicited offers; badges never auto-focus/flip. Hotkeys: `HotkeyController` in-call only — `project_issue38_watch_gate_ptt_grid`.
- **CRITICAL — mic gain/loudness:** WebRTC APM AGC DISABLED — NEVER re-enable. Voice Enhancement owns loudness (`setCaptureGain`/`setVoiceEnhance`; `setVolume()`=NO-OP): chain+servo → −16 dBFS, 3 ports IN SYNC, tune ONLY via the g++ harness. NEVER a per-sample leveler, NEVER bypass iOS VPIO. `project_voice_agc_loudness_rvox`.
- **AI noise suppression LIVE:** RNNoise default (DFN3 = desktop selector), ABI v3; WebRTC NS auto-disabled; RNNoise VAD gates upward boost; `frames>0` or the test didn't happen; capture buffers ALWAYS fullband mono. `project_dfn3_noise_suppression`, `project_voice_enhance_chain`.
- **CRITICAL — SFrame:** `setKeyIndexForPeer` after every enable; `rotateKey` (not `setSharedKey`) in `setSframeKey`; cryptors idempotent per (peer,kind) — a reneg REPLACING a sender must drop+re-enable BOTH ends. Enables SYMMETRIC + self-healing; a commit evicting OUR leaf = INACTIVE group → re-bootstrap. `project_sframe_heal_ladder`.
...
- **CRITICAL — media forwarder lane (`fwd_*`, own `fwd:{id}` room; step 3 phase 1 SHIPPED):** legs take **ZERO ice servers** (configured STUN makes libwebrtc withhold ALL candidates), EXEMPT from Always-relay; PT translation matches the FULL `CodecSpec` (VP9 has 2 profiles → wrong PT = black screen, healthy counters); keyframe requests need the SSRC fallback + coalesce-never-drop. `project_media_forwarding_epic`.
