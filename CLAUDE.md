...
├── relay-uws/            # Production relay (uWebSockets C++, native TLS, OVH VPS); relay/ = legacy
├── packages/flutter_webrtc/ # FORK 1.5.2, libwebrtc m144 = OUR patched build;
│                         # screen_audio_capturer.exe = SEPARATE target.
...
```
**Certum signing:** scripts self-heal the CNG binding; output `installer\Output\` (gitignored). See `reference_certum_signing_procedure`.
...
- **CRITICAL — Profile Showcase Board:** wire field `Option<String>` (absent = old client → PRESERVE; `""` = clear); NO relational blocks (VETOED); IGDB authoring-only via `igdb/search.php` POST-only; images LOSSY WebP; fetchers refuse non-Hollow-CDN URLs. See `project_showcase_board_impl`.
...
- **Window chrome:** `window_manager`, `setAsFrameless()`, 32px title bar.
...
- **Performance sentinels (self-diagnosis):** quiet-by-default anomaly loggers, grep `[SENTINEL]` in hollow_debug.log; direct-channel call sites use `timedChannelCall`. See `project_perf_sentinels`.
...
- **CRITICAL — relay per-IP accounting goes through `ip_limit_key()`** (v4 addr / v6 **/64**; unmap v4-MAPPED `::ffff:…` FIRST — naive /64 truncation collapses ALL v4 into ONE bucket). 10 GB/day budget counts binary BOTH directions; exhaustion = explicit `1008 "bandwidth_limit"` close, never silent. See `project_relay_bandwidth_enforcement`, `project_relay_ipv6`.
- **CRITICAL — relay offline delivery (availability cache, ON 3d):** anything reaching OFFLINE channel members MUST ride `0x07` topic frames — `0x03` and direct sends are invisible to the TTL rings. MLS groups need `hollow_join_config`. OPEN BUG: channel files don't render post-catchup. See `project_relay_availability_cache`.
...
- **CRITICAL — WS sends: bounded + never silently discarded.** ALL ws_client.rs sink writes go via `bounded_send` (30s timeout) — a raw `write.send()` on a wedged socket pends FOREVER and freezes the select loop INCLUDING the liveness watchdog. `send_command()` false → break, buffer, reconnect. See `feedback_ws_zombie_liveness_timeout`.
...
- **CRITICAL — multi-device device-vs-master routing.** One MASTER = many DEVICE peer_ids. Iron rules: (1) per-person UI/lookups collapse device→master (Rust `resolver::resolve`; Dart `identityOf`); (2) SENDS target a DEVICE, never the bare master (no socket → silently dropped) — fan via `devices_for(master)`. `dm_room_code` is PURE — never resolve inside it. Fresh installs ALWAYS have device≠master. See `feedback_multidevice_targeting_sweep`.
...
- **CRITICAL — multi-device data channels (`webrtc_service.dart`, hollow-data):** glare tiebreaker compares MASTER identities; answer/ICE match by `conn_id`, never peer_id; sends/sockets/map keys stay DEVICE-keyed. `sendScreenAudio` DROPS packets when `bufferedAmount` >256KB (`feedback_screen_audio_datachannel_backpressure`). VC mesh glare is the OPPOSITE: ROUTABLE DEVICE ids, never master (`feedback_vc_join_double_announce_race`).
...
- **CRITICAL — channel push:** sender fans one `0x09` frame per OFFLINE member (targets + mention flag from its CRDT — relay never learns membership); relay filters `set_push_prefs` (MUST be relay-side) + debounce; fetch node decrypts via `from_persisted`. Prefs sync MOBILE-ONLY, text-only v1. See `project_channel_push_notifications`.
...
- **CRITICAL — mic gain/loudness:** WebRTC APM AGC DISABLED — NEVER re-enable. Voice Enhancement owns loudness (`setCaptureGain`/`setVoiceEnhance`; `setVolume()`=NO-OP): chain+servo → −16 dBFS, 3 ports IN SYNC, tune ONLY via the g++ harness. NEVER a per-sample adaptive leveler (crackles), NEVER bypass iOS VPIO. See `project_voice_agc_loudness_rvox`.
- **AI noise suppression LIVE:** RNNoise default (DFN3 = desktop selector), ABI v3; WebRTC NS auto-disabled; RNNoise VAD gates upward boost; `frames>0` or the test didn't happen; capture buffers ALWAYS fullband mono (`rate/100`-sized = bands=1). See `project_dfn3_noise_suppression`, `project_voice_enhance_chain`.
...
- **CRITICAL — CRDT property changes: optimistic UI update BEFORE the fire-and-forget FFI, and NEVER a read-back right after the write returns** (set_* only queues — the read sees the PREVIOUS value; seed the cache via `applyLocalWrite`). See `feedback_crdt_optimistic_update`, `feedback_crdt_read_after_write_race`.
...
- **CRITICAL — channel visibility/posting UI reactivity:** `_refreshServerState` reloads `channelListProvider` + invalidates `serverChannelsProvider(id)` on a retry ramp; eviction listeners on `visibleChannelsProvider`; mobile chat route must `loadForServer` on open. NEVER `ref.invalidate` in `initState` — defer to post-frame. See `feedback_channel_visibility_posting_ui_reactivity`.
