...
- **CRITICAL — custom emotes:** content-addressed WebP, wire token `[e:name:hash]` (grammar dual-defined: `node/emotes.rs` + `emote_image.dart`); bytes NEVER ride CRDT/envelopes/relay rings — pull via `EmoteRequest`→`EmoteAssets` (verified). FFZ = AUTHORING-ONLY via the `ffz/` proxy; receivers never fetch. Composer: 1 PUA char ↔ 1 WidgetSpan; send sites read `expandedText()`, NEVER `.text`. Notif previews use `emotePreviewSpans`/`emoteTokensToShortcodes` (Rust twin for NSE), never raw text. See `project_custom_emotes_ffz`, wiki `emotes`.
