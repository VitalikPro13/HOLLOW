...
- **CRITICAL — Profile Showcase Board:** `showcase_board` wire field is `Option<String>` (absent = old client → PRESERVE; `""` = clear); asset bundle rides the avatar blob playbook. NO relational blocks (VETOED). IGDB at AUTHORING only via `igdb/search.php`, POST-only; deprecated IGDB enums return NOTHING silently. Images = LOSSY WebP only; fetchers refuse non-Hollow-CDN URLs. See `project_showcase_board_impl`.
- **CRITICAL — custom emotes:** every emote = content-addressed WebP blob; wire token `[e:name:hash]` (grammar dual-defined: `node/emotes.rs` + `emote_image.dart`); bytes NEVER ride CRDT/envelopes/relay rings — pull via `EmoteRequest`→`EmoteAssets`, verified at receive. FFZ = AUTHORING-ONLY via the website `ffz/` proxy; receivers never make HTTP requests for emotes. Server sets gated by `MANAGE_EMOTES` (1<<7). See `project_custom_emotes_ffz`, wiki `emotes`.
