...
- **Deep links:** `hollow://` on all 6 platforms via `DeepLinkService` (init pre-runApp, buffers until shell ready). Invites COPY `webServerInviteLink()` (`…/join#server=ID` — FRAGMENT, id never hits server logs); `classifyHollowLink` normalizes both forms; Windows forward = NO-ARG `SendAppLinkToInstance()` in main.cpp. See `project_deep_linking`.
