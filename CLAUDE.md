...
- **Relay:** uWebSockets C++ (`relay-uws/`) on OVH VPS. NO rate limits; NEVER lower `maxPayloadLength` (64MB). Topics `0x07`, broadcast `0x03`. EVERY binary handler gates on SENDER room membership; abuse = fair-share eviction, NEVER a cap. `feedback_relay_rules`, `feedback_relay_no_metadata_logging`.
...
- **CRITICAL — targeted sends route to their DETERMINISTIC room, never `ws_room_for_peer`** (first-match → silent one-way loss): DMs → `dm_room_code`, forwarder signals → `fwd:{id}`. Sibling self-echo keeps the flexible lookup. Friend requests queued on a COLD resolver drain on device-list ingest, not just presence. `feedback_dm_friend_establishment_bugs_2026_07`.
...
- **CRITICAL — Windows mid-call media:** `addTrack`/`removeTrack` + renegotiate — NEVER `replaceTrack`; live sender `setParameters` ALWAYS REJECTED on Win → encodings ride `addTransceiver` init ONLY, cap changes re-offer. `feedback_webrtc_patterns`.
...
- **CRITICAL — media forwarder lane (`fwd_*`, own `fwd:{id}` room; phases 1+2 SHIPPED, field-verified):** CLIENT legs = **ZERO ice servers** (configured STUN withholds candidates), EXEMPT from Always-relay; PT translation = FULL `CodecSpec`; keyframes = SSRC fallback + coalesce-never-drop. Peer forwarders: ONE fwd_* contract, expectation gate = fwd_capable watch, assign trust = ORIGINATOR — EXCEPT Always-relay users' media never touches a MEMBER's machine either role (`relay_private` + viewer hard-refusal; VPS = TURN trust domain OK); `peer_media_forwarding` default ON post-node-start. **NO rate limiter on the fwd control plane** (silent-drop class — never re-add); presence = weak signal, MEDIA LEGS only truth. `project_media_forwarding_epic`.
...
- **CRITICAL — message dedup is by message_id, NEVER content:** every insert pre-checks `dm/channel_message_exists(mid)`; content UNIQUE indexes LEGACY-only. Receive events ALWAYS emit a `duplicate` flag — never suppress emission on "row exists". Sync `since` = watermark − `SYNC_LOOKBACK_MS`. `feedback_sync_dedup_watermark_events`.
...
- **CRITICAL — connection indicators read `overallConnectionProvider` ONLY** (both user bars via `connectionVisual()`) — never node status (green with no internet), never who ELSE is online; sync may refine "Online", never contradict it. Header `Offline` = OUR link down; empty room = `ConnectionStage.alone`. `feedback_genuine_connection_status`.
...
- **CRITICAL — server-group MLS recovery:** owner-preferred coordinator + CRDT-mutating handlers ALWAYS also broadcast the op as plaintext `CrdtOpBroadcast` (idempotent; op_log dedups) — an MLS-only op silently drops at a skewed epoch. `WrongEpoch` decrypt-fail fires a server-group `SyncRequest`. `feedback_owner_coordinator_mls_recovery`.
