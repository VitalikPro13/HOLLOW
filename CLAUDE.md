...
- **Native screen share (macOS + Windows):** macOS: `FlutterScreenCaptureKitCapturer` in `packages/flutter_webrtc/macos/Classes/` feeds NV12 frames via `RTCVideoCapturerDelegate`. Windows: `WinScreenShareCapturer` in `packages/flutter_webrtc/windows/` uses D3D11 Video Processor (GPU BGRA→NV12) + custom libwebrtc DLL with `RTCVideoSource::OnCapturedFrame()`. Pipeline: WGC BGRA texture → GPU VP Blt → NV12 staging → libyuv NV12→I420 → encoder. Custom DLL built from `D:\libwebrtc-build\` (m144 + PR #138). See `NATIVE_SCREEN_CAPTURE_RESEARCH.md` for the next phase (MF hardware encoder + process audio capture).
- **CRITICAL — custom libwebrtc DLL:** The Windows flutter_webrtc fork requires a custom-built `libwebrtc.dll` (not the prebuilt one from flutter-webrtc releases). Built from `D:\libwebrtc-build\src\` with `ninja -C out/Windows-x64 libwebrtc:libwebrtc`. The DLL adds `OnCapturedFrame`, `CreateCustomVideoSource`, `CreateFromBGRA`, `CreateFromNV12`. The prebuilt DLL's `RTCVideoSource` is empty (no methods). The custom DLL is gitignored — must be rebuilt locally after cloning.
