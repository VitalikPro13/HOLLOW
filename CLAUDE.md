...
- **`.hollowpack` (artist shop):** identity = sha256 of PROCESSED bytes; the CLI = `rust/hollow_art` (`#[path]`-includes the app's encoders; musl build on the shop host), NEVER a lookalike; `import_hollowpack` verifies caps/hashes/dims/centre, NEVER re-encodes. wiki `hollowpack`.
...
- **MLS coordinator:** lowest online peer_id; server-group ops PREFER the OWNER as COMMITTER only; epoch catch-up excludes the peer that is BEHIND (`epoch_catchup_responder`). Commits ONLY via `broadcast_mls_commit` (one SendToRoom + epoch guard, never per-device loops); plaintext twin via `broadcast_crdt_op_to_members`. `feedback_owner_coordinator_mls_recovery`.
