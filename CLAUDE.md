...
- **CRITICAL — never use raw `OverlayEntry` inside `SelectionArea` or archive viewers.** `SelectionArea` creates a nested overlay scope for text selection handles. Inserting a raw `OverlayEntry` (even with `rootOverlay: true`) crashes with `Null check operator used on a null value` at `_OverlayEntryWidgetState.initState` because the entry's `_overlayEntryStateNotifier` is null. **Pattern:** use `showDialog` with `barrierColor: Colors.transparent` + `Align` + `Padding` to position the popup. Pop with `Navigator.of(context).pop(value)`. See `_FilterDialog` in `archive_message_viewer.dart`.
- **`scrollable_positioned_list: ^0.3.8`** — used in `chat_pane.dart`, `channel_chat_pane.dart`, and all archive message viewers. Sentinel pattern: `itemCount: messages.length + 1`, last item = `SizedBox.shrink()`, `initialScrollIndex: messages.length, initialAlignment: 1.0` for bottom anchoring. `_scrollToMessage(int index)` uses `_itemScrollController.scrollTo(index: index, alignment: 0.3)`. Non-reversed chronological order. Reply-tap and search-result navigation both use index-based scrolling. Package was removed once before and broke everything — do not remove again.
