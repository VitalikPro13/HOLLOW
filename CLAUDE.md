...
- **Accessibility (epic CLOSED), iron rules:** icon-only controls need a PURPOSE label; focus rides `HollowFocusRing` (both CI-guarded); reduce motion ONLY via `ReduceMotionController` + `hollowMobileRoute()`; larger text = the 3 safe bar-fix patterns; StatusDot = SHAPE (`filled:`); never `dart format` mid-edit — `project_accessibility_*`.
...
- **Storage layout:** data root = `dirs::data_dir()/hollow` (`keys.rs::data_dir`; override via `set_data_dir()`/`HOLLOW_DATA_DIR`; NOT `~/.hollow`); mobile = sandboxed app documents (iOS in the App Group). Subfolders: `files/`, `vault/`, `vault_cache/` + `identity.key`/`identity.device` + `messages.db`.
...
- **AI noise suppression LIVE:** RNNoise default (DFN3 = desktop Engine chip), `hollow_dfn`+`dfn_ffi.rs` ABI v3, WebRTC NS auto-disabled + fallback; RNNoise VAD gates upward boost; `frames>0` or the test didn't happen. Capture buffers = ALWAYS fullband mono (`num_bands`=APM metadata) — AI-NS ports AND the enhance chain treat `rate/100`-sized buffers as bands=1 (48k devices get the full chain). See `project_dfn3_noise_suppression`, `project_voice_enhance_chain`.
...
- **CRITICAL — audio-track `setEnabled`/`setVolume` = blocking signaling-thread hops** (LiveKit releases the mic while audio tracks are disabled → 2s UI freeze): run via `audioTrackOpExecutor` (Android) / `HollowAudioTrackOpQueue` (darwin), never on the platform thread. FRB's TRACE device loggers capped at Warn in `init_app` — NEVER remove. See `feedback_android_audio_track_proxy_ui_freeze`, `feedback_frb_trace_logger_cap`.
