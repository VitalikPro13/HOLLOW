...
- **Perf sentinels:** grep `[SENTINEL]`; `timedChannelCall`; `FrameCensus` (fps + build/raster), `FrameScheduleProbe` (who called `scheduleFrame`, works in RELEASE); `scripts/perf_*.ps1`. `project_perf_sentinels`, `project_idle_cpu_frame_scheduling`.
