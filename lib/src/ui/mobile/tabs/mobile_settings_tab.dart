  ...
  Future<void> _save() async {
    ...
    try {
      ...
      await ref.read(profileProvider.notifier).updateMyProfile(
        ...
        twitchUsername: ref
                .read(profileProvider)[ref.read(identityProvider).peerId ?? '']
                ?.twitchUsername ??
            '',
...
class _TwitchRowState extends ConsumerState<_TwitchRow> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _username = username;
          _verifiedLogin = me == null ? null : ref.read(twitchLoginProvider(me));
          _loading = false;
        });
      }
    } catch (_) {
  ...
  Future<void> _verifyAccount() async {
    ...
    setState(() => _busy = true);
    try {
      final outcome = await twitch_api.twitchVerifyOwner();
      if (!mounted) return;
      setState(() {
      ...
      HollowToast.show(
        context,
        outcome.verified
            ? 'Twitch verified as ${outcome.login}'
            : outcome.message,
  ...
  Future<void> _disconnect() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _busy = false);
        HollowToast.show(context, 'Failed to disconnect', type: HollowToastType.error);
      }
    }
  }
              ...
              children: [
                ...
                Text(
                  _verifiedLogin != null
                      ...
                      : _connected
                          ? '$_username, not verified yet'
                          : 'Not connected',
                  style: HollowTypography.caption.copyWith(
                    color: _verifiedLogin != null
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: const [
        ...
        SizedBox(height: HollowSpacing.xl),
        HollowSectionHeader('Messages'),
        MessageDisplayPicker(),
        SizedBox(height: HollowSpacing.xl),
