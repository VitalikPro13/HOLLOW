...
/// asset rail, pulled on demand and LRU-evicted under the Storage Manager cap,
/// while the STILL companion stays inside the profile blob. That split is the
/// bandwidth fix: a profile update is PUSHED to everyone who syncs with you,
/// so before this every reconnect re-shipped megabytes of unchanged GIF, and
/// an eviction here costs a re-pull rather than a blank face.
///
/// Avatar and banner share one cache and one `AssetKind` on purpose — they
/// share a replication profile (one of each per person you have ever met), so
...
class ProfileAnimNotifier extends Notifier<Map<String, Uint8List>> {
  ...
  /// asset rail asks ONCE per session, so media whose owner was offline would
  /// otherwise stay still for the rest of the session; a fresh profile
  ...
  void ensure(String hash, {String? peerHint}) {
    if (!isProfileAnimHash(hash) ||
        ...
        // every avatar we don't hold fires a fresh SQLCipher query across the
        // FFI. [onAssetsReceived] and [onProfileUpdated] are what clear it.
  ...
  /// — this is the reconnect path (see [_awaiting]).
...
/// [peerId]'s animated AVATAR bytes, or null when they have no animated
/// variant or we don't hold it yet — callers keep painting the still, which
/// is exactly what they should do while a pull is in flight.
///
/// Safe to call from `build`: the pull is deferred.
